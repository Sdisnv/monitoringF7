...
class ScopePdfRenderer {
  ...
  paraWithBoldRate(text){
    ...
    if(!match){
      this.para(text, { size, align: 'justify' });
      return;
    }
    const idx = match.index;
    this.doc.fillColor(rgb(INSTITUTION.ink)).font('Helvetica').fontSize(size)
      .text(text.slice(0, idx), MARGIN, this.doc.y, { width, continued: true, align: 'justify' });
    this.doc.font('Helvetica-Bold').text(match[0], { continued: true, align: 'justify' });
    this.doc.font('Helvetica').text(text.slice(idx + match[0].length), { continued: false, width, align: 'justify' });
  }
  ...
  renderCycleReportBody(m){
    const cr = m.cycleReport || {};
    const cycle = cr.cycle || {};
    const k = cr.kpis || {};
    const period = `${formatDisplayDate(m.period && m.period.from)} - ${formatDisplayDate(m.period && m.period.to)}`;
    this.renderReportTitle(
      String(m.title || 'Rapport de cycle').toLocaleUpperCase('fr-CH'),
      cr.typeLabel || 'Cycle',
      `${period} · ${cr.statusLabel || '—'}`
    );
    this.kv([
      { label: 'Type', value: cr.typeLabel || 'Cycle' },
      { label: 'Formation / cycle', value: cycle.libelle || '—' },
      { label: 'Domaine', value: domaineLabel(m.domaine) || m.domaine || '—' },
      { label: 'Période', value: period },
      { label: 'Sessions', value: `${cr.sessionsRealised || 0}/${cr.sessionCount || 0} réalisées` },
      { label: 'Population', value: String(k.population || 0) },
      { label: 'Statut', value: cr.statusLabel || '—' },
  ...
  renderSessionBody(m){
    ...
    if(donutPoints.length){
      this.doc.y = this.drawDonutLegend(donutPoints, leftX, this.doc.y, colW);
    }
    if(m.graphs && m.graphs.volumesSeances && (m.seances || []).length){
      this.doc.y += TYPE.blockShift;
      this.doc.fillColor(rgb(INSTITUTION.ink)).font('Helvetica-Bold').fontSize(8)
  ...
  renderJspReportBody(m){
    ...
    if(enabled('comparaisons') && (jsp.siteRows || []).length){
      ...
      this.table(
        ['Périmètre', 'Pers.', 'Exercices', 'Attendus', 'Présents', 'Excusés', 'Absents', 'Taux'],
        (jsp.siteRows || []).map((row) => [
          row.site, String(row.participants || row.jeunes || 0), String(row.exercises || 0), String(row.expected || 0),
          String(row.present || 0), String(row.excused || 0), String(row.absent || 0), formatTaux(row.presenceRate)
        ]),
        [68, 32, 55, 55, 55, 55, 55, 62],
    ...
    if(enabled('surveillance') && (jsp.watchlist || []).length){
      ...
      this.table(
        ['Personne', 'Périmètre', 'Att.', 'Prés.', 'Exc.', 'Abs.', 'Absences', 'Taux abs.'],
    ...
    if(enabled('regularite') && (jsp.regulars || []).length){
      this.heading('Participation régulière', 12);
      this.table(
        ['Personne', 'Périmètre', 'Att.', 'Prés.', 'Exc.', 'Abs.', 'Taux'],
    ...
    if(enabled('motifs') && (jsp.motifs || []).length){
  ...
  render(){
    ...
    if(m.kind === 'SESSION'){
      ...
      this.para(`Établi le : ${formatDisplayDate(this.meta.generatedAt) || formatDisplayDate(m.event.date)}`);
      this.doc.moveDown(0.55);

...
const GUARD_SQL = `create or replace function scope_activity_definition_version_guard()
...
const CHILD_TABLES = Object.freeze([
  'scope_activity_domain_bindings','scope_activity_session_templates','scope_activity_periodicities',
  'scope_activity_qualification_bindings','scope_activity_role_requirements','scope_activity_location_requirements',
  'scope_activity_responsible_requirements','scope_activity_planning_constraints','scope_activity_statistical_contributions'
]);

function childTriggerSql(table){
  return `drop trigger if exists ${table}_immutable_trg on ${table}; create trigger ${table}_immutable_trg before insert or update or delete on ${table} for each row execute function scope_activity_version_child_guard()`;
}
...
module.exports = { DDL,GUARD_SQL,CHILD_TABLES,childTriggerSql,PROTECTION_SQL };

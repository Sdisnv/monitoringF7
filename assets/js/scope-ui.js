...
(function () {
  ...
  const state = {
    ...
    scopeSearchTimers: {},
    scopeSearchTokens: { encadrement: 0, manual: 0, preview: 0 },
    manualPersonQuery: '',
    manualPersonHits: [],
    reopenMotif: '',
    session: null,
    ...
    quoVadisArbitrerOpenDomains: null,
    quoVadisArbitrerTouched: false,
  ...
  function domainTaxonomySelectHtml(selected, options = {}) {
    ...
    const groups = L.domainTaxonomyGroups();
    return `<option value="${escapeHtml(String(emptyValue))}" ${String(selectedValue) === String(emptyValue) ? 'selected' : ''}>${escapeHtml(emptyLabel)}</option>`
      + groups.map((group) => `<optgroup label="${escapeHtml(group.label)}">${group.codes.map((code) => `${code === group.separatorBefore ? '<option disabled>────────</option>' : ''}<option value="${escapeHtml(code)}" ${String(selectedValue) === code ? 'selected' : ''}>${escapeHtml(code)}</option>`).join('')}</optgroup>`).join('');
  }
  ...
  async function loadQuoVadis() {
    if (typeof client.quoVadisProgramme !== 'function') {
      ...
      state.quoVadisError = 'QUO VADIS indisponible.';
      return null;
    }
  ...
  async function loadQuoVadisHistory(activityId) {
    if (!activityId || typeof client.quoVadisActivityReferences !== 'function') {
      state.quoVadisHistory = [];
      state.quoVadisHistoryReady = true;
      state.quoVadisHistoryError = null;
      state.quoVadisHistoryId = activityId || '';
      return [];
    }
    if (state.quoVadisHistoryId === String(activityId) && state.quoVadisHistoryReady) return state.quoVadisHistory;
    state.quoVadisHistoryReady = false;
    state.quoVadisHistoryError = null;
    state.quoVadisHistoryId = String(activityId);
    try {
      const data = await client.quoVadisActivityReferences(activityId, 2027);
      state.quoVadisHistory = data.references || [];
      state.quoVadisHistoryReady = true;
      return state.quoVadisHistory;
    } catch (error) {
      state.quoVadisHistory = [];
      state.quoVadisHistoryReady = true;
      state.quoVadisHistoryError = L.friendlyError(error).message || 'La référence 2026 n’a pas pu être chargée.';
      return [];
    }
  }

  async function loadPersonCount() {
    if (!client.personnesCount && !client.listPersonnes) return null;
    const data = await cached('personCount', {}, () => client.personnesCount ? client.personnesCount() : client.listPersonnes());
    state.personCount = Number(data.count != null ? data.count : ((data.personnes || []).length));
  ...
  async function loadFiche(id) {
    ...
    const ev = data && data.evenement;
    const mode = ev ? eventMode(ev) : '';
    if (
      ev
      && String(ev.statut || '').toUpperCase() === 'PLANIFIE'
      && !ev.population_figee
      && mode !== 'QUANTITATIF'
  ...
  function encadrementPersonSummary(p) {
  ...
  function sidebarHtml(r) {
    ...
    const canSee = (item) => !item.permission || hasScopePermission(item.permission);
    ...
    const renderGroup = (group) => {
      if (!group) return '';
      const groupContent = group.sections
        ? group.sections.map((sub) => navSubsection(sub.label, (sub.items || []).filter(canSee))).join('')
        : (group.items || []).filter(canSee).map((item) => link(item, item.current)).join('');
      if (!groupContent) return '';
      const expanded = state.openGroups[group.id] != null ? state.openGroups[group.id] : group.current;
      return `<div class="scope-nav-group${expanded ? '' : ' is-collapsed'}">
        <button type="button" class="scope-nav-group-head${group.current ? ' is-current' : ''}" data-nav-group="${escapeHtml(group.id)}" aria-expanded="${expanded ? 'true' : 'false'}">
          ${navIcon(group.icon)}
          <span>${escapeHtml(group.label)}</span>
        </button>
        <div class="scope-nav-sub">${groupContent}</div>
      </div>`;
    };
    const groupById = new Map((model.groups || []).map((group) => [group.id, group]));
    const directLinks = (model.direct || []).filter(canSee).map((item) => link(item, item.current)).join('');
    return `
      ...
      <aside class="scope-sidebar" id="scope-sidebar" aria-label="Navigation principale" aria-modal="${state.navOpen ? 'true' : 'false'}">
        ...
        <nav class="scope-nav-scroll">
          ${section('Accueil')}
          ${link(model.home || { href: '#/accueil', label: 'Accueil', icon: 'home', current: r.screen === 'accueil' }, model.home ? model.home.current : r.screen === 'accueil')}
          ${section('Activité')}
          ${renderGroup(groupById.get('activite'))}
          ${section('Pilotage')}
          ${renderGroup(groupById.get('pilotage'))}
          ${directLinks}
          ${section('Administration')}
          ${renderGroup(groupById.get('administration'))}
        </nav>
      ...
      <div class="scope-main scope-home">
        ...
        <section class="scope-quick" aria-label="Accès rapides">
          ...
          <div class="scope-quick-grid">
            ...
            <a href="#/statistiques"><span class="scope-quick-ico">${homePilotIcon('analyses')}</span><span><b>Analyses</b><small>Graphiques et résultats de période</small></span><span class="scope-quick-chev" aria-hidden="true">›</span></a>
  ...
  function renderStatistiques() {
    ...
    if (!dash && !state.dashboardError) {
      return `<div class="scope-crumb">Analyses</div><div class="scope-main">${pageHeaderHtml({ eyebrow: 'Pilotage', title: 'Analyses', context: state.year, logo: true })}<div class="scope-card scope-placeholder"><p>${escapeHtml(L.loadingMessage('dashboard'))}</p></div></div>`;
    }
    if (state.dashboardError) {
      return `<div class="scope-crumb">Analyses</div><div class="scope-main">${pageHeaderHtml({ eyebrow: 'Pilotage', title: 'Analyses', context: state.year, logo: true })}<div class="scope-card scope-placeholder"><p class="scope-state-error" role="alert">${escapeHtml(state.dashboardError)}</p></div></div>`;
    }
    ...
    return `
      <div class="scope-crumb">Analyses</div>
      <div class="scope-main scope-analyses-page">
        ${pageHeaderHtml({ eyebrow: 'Pilotage', title: 'Analyses', context: periodLabel(dash.period), description: 'Lecture consolidée des résultats SCOPE issue des moteurs Analytics, Objectifs, Cycles et Vigilance.', logo: true })}
        ${periodContextHtml()}
  ...
  function renderListe() {
    ...
    return `
      ...
      <div class="scope-main">
        ...
        <p class="scope-mode-hint">La création manuelle reste disponible pour un événement ponctuel. Les imports sont regroupés dans Administration → Imports.</p>
  ...
  function eventReportAvailability(ev, fiche) {
    if (!ev) return { ok: false, hide: true, reason: 'Rapport indisponible.' };
    const statut = String(ev.statut || '').toUpperCase();
    if (statut === 'ANNULE') return { ok: true, hide: false, reason: '' };
    if (statut === 'REALISE') return { ok: true, hide: false, reason: '' };
    if (ev.origine === 'LEGACY_AGGREGATED') return { ok: true, hide: false, reason: '' };
    if (eventMode(ev) === 'QUANTITATIF') {
      const saisie = fiche && (fiche.saisieQuantitative || fiche.saisie);
      return saisie
        ? { ok: true, hide: false, reason: '' }
        : { ok: false, hide: false, reason: 'Aucune donnée exploitable pour ce rapport.' };
    }
    if (!ev.population_figee) {
      return { ok: false, hide: true, reason: 'Assignez d’abord les participants.' };
    }
    const hasData = ((fiche && fiche.participations) || []).some((row) => {
      const value = String(row && row.statut || '').toUpperCase();
      return value && value !== 'NON_RENSEIGNE' && value !== 'NON_CONCERNE';
    });
    if (!hasData) return { ok: false, hide: false, reason: 'Aucune donnée exploitable pour ce rapport.' };
    return { ok: true, hide: false, reason: '' };
  }

  function reportButton(id, fiche) {
    const ev = (fiche && fiche.evenement) || (state.fiche && state.fiche.evenement) || { evenement_id: id };
    const availability = eventReportAvailability(ev, fiche || state.fiche);
    if (availability.hide) return '';
    if (!availability.ok) {
      return `<button type="button" class="scope-btn scope-btn-secondary scope-btn-compact" data-report-event="${escapeHtml(id)}" disabled aria-disabled="true" title="${escapeHtml(availability.reason)}">Générer le rapport</button>`;
    }
  ...
  function renderObjectifs() {
    ...
    return `
      <div class="scope-crumb">Administration / Objectifs</div>
      <div class="scope-main">
        ${pageHeaderHtml({
          eyebrow: 'Administration / Application',
  ...
  function formatPlannedDurationLabel(startValue, endValue) {
  ...
  function renderConfigCompatibility(item) {
    if (!item) return '';
    ...
    const organisation = String(version.mode_organisation || version.modeOrganisation || 'SIMPLE').toUpperCase() === 'MULTI_SESSION'
      ? `Plusieurs sessions · ${sessions} sessions`
      : 'Session unique';
    const period = [from ? `du ${L.formatDate(from)}` : '', to ? `au ${L.formatDate(to)}` : ''].filter(Boolean).join(' ');
    return `<div class="scope-config-summary">
      <strong>${escapeHtml(definition.label || 'Configuration de formation')}</strong>
      <span>${escapeHtml(organisation)}</span>
      <span>${escapeHtml(period || 'Période ouverte')}</span>
      <small>Compatible avec le domaine sélectionné, la date de l’événement et une version active.</small>
    </div>`;
  }
        ...
        <div class="scope-card scope-event-form-card">
          ...
          <div class="scope-event-form-grid">
            <section class="scope-form-section is-wide">
              ...
              <div class="scope-identification-grid">
            ...
            <section class="scope-form-section">
              ...
              ${multiMode ? `
                ...
                ${selectedConfig ? `<p class="scope-mode-hint">Cette configuration prévoit ${escapeHtml(String(sessionCount || 2))} sessions.</p>` : ''}
                <p class="scope-mode-hint">${selectedConfig ? 'Pour un autre nombre de sessions, choisissez une autre configuration ou le mode Multi-session manuel.' : 'Une participation valide à l’une des sessions satisfait la formation.'}</p>
    ...
    return `
      ...
      <div class="scope-main">
        ...
        <div class="scope-card" style="margin-top:12px">
          ...
          <p class="scope-mode-hint">PR et AUTO sont des domaines de formation.</p>
          ${tree.map((d) => `
  ...
  function scopeInlineIcon(kind) {
    if (kind === 'watch') {
      return `<svg class="scope-inline-ico" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><circle cx="12" cy="12" r="8.2" fill="none" stroke="currentColor" stroke-width="1.7"/><path d="M12 7.6v4.7l3.1 1.7" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>`;
    }
    return `<svg class="scope-inline-ico" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><circle cx="12" cy="13" r="7.2" fill="none" stroke="currentColor" stroke-width="1.7"/><path d="M12 10.2v3.4l2.3 1.3M9.4 5.2h5.2" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>`;
  }

  function formatEventClock(value) {
    return (L.formatClockLabel && L.formatClockLabel(value)) || '';
  }
  ...
  function eventTemporalSummaryHtml(ev, fiche) {
    ...
    return `<p class="scope-fiche-time-line">
      <span>Prévu : ${escapeHtml(formatEventClock(plannedStart) || plannedStart)}–${escapeHtml(formatEventClock(plannedEnd) || plannedEnd)}</span>
      <span>Réalisé : ${escapeHtml(formatEventClock(actualStart) || actualStart)}–${escapeHtml(formatEventClock(actualEnd) || actualEnd)}</span>
      ${durationText ? `<span>${scopeInlineIcon('chrono')} Durée réalisée : ${escapeHtml(durationText)}</span>` : ''}
    </p>`;
  }
  ...
  function renderFichePrimaryAction(cta, lifecycleActions = '') {
    ...
    return `<section class="scope-card scope-fiche-section scope-fiche-primary">
      ...
      <div class="scope-actions scope-event-toolbar scope-fiche-primary-actions">
        ${cta ? `<button type="button" class="scope-btn scope-btn-primary scope-fiche-cta" data-cta="${cta.action}"${(cta.action === 'figer' && state.participantAssignmentBusy) || (cta.action !== 'figer' && state.loading) ? ' disabled' : ''}>${escapeHtml(cta.action === 'figer' && state.participantAssignmentBusy ? 'Assignation…' : cta.label)}</button>` : ''}
        ${lifecycleActions || ''}
      </div>
    </section>`;
  }
  ...
  function renderFicheSecondaryActions(ev, extraActions, isLegacy) {
    return `<section class="scope-card scope-fiche-section scope-fiche-secondary">
      ...
      <div class="scope-actions scope-event-toolbar scope-fiche-secondary-actions">${extraActions || ''}${reportButton(ev.evenement_id, state.fiche)}</div>
    </section>`;
  }
  ...
  function previewInclusionLabel(p) {
    if (p.motifInclusion === 'exception_ajout' || p.origine === 'EXCEPTION_AJOUT' || p.manual) return 'Ajout manuel';
  ...
  function addPreviewPerson(id) {
    const person = (state.personHits || []).find((p) => String(p.personne_id) === String(id));
    if (!person) {
  ...
  function renderEditEventModal(ev, fiche) {
    ...
    return `<div class="scope-modal"><div class="scope-card scope-edit-event-modal">
      ...
      <section class="scope-form-section">
        <h4>Public cible</h4>
        <small class="scope-target-help">Les choix proviennent des référentiels SCOPE du domaine de l’événement.</small>
        ${renderTargetPicker('edit-event-cibles', cibles, selected, { disabled: cibleDisabled })}
      </section>
      <div class="scope-field"><label>Motif de modification</label><textarea id="edit-event-motif">${escapeHtml(form.motif || '')}</textarea></div>
      ${warning}
      <div class="scope-actions">
      <button type="button" class="scope-btn scope-btn-primary" id="edit-event-ok"${state.eventSaveBusy ? ' disabled' : ''}>${state.eventSaveBusy ? 'Enregistrement…' : (form.confirmed ? 'Confirmer et enregistrer' : 'Enregistrer les modifications')}</button>
  ...
  function previewSortColumns() {
    return [
      ...
      { key: 'origine', type: 'text', value: (p) => previewInclusionLabel(p) }
    ];
  }
      ...
      return `<tr>
        <td data-label="Sélection" class="scope-preview-select-cell">
          <label class="scope-preview-check">
            <input type="checkbox" data-preview-select="${escapeHtml(personneId)}" ${selected ? 'checked' : ''} aria-label="Sélectionner">
          </label>
        </td>
        ...
        <td data-label="Origine">${escapeHtml(previewInclusionLabel(p))}</td>
      </tr>`;
    ...
    return `<div class="scope-table-wrap scope-fiche-preview-wrap">
      <table class="scope-table scope-fiche-preview-table">
        <thead><tr>
          ...
          ${sortableHeader('event-preview', 'origine', 'Origine', state.previewSort)}
        </tr></thead>
      ...
      <section class="scope-card scope-fiche-section scope-fiche-preview">
        ...
        <div class="scope-toolbar scope-fiche-preview-toolbar">
          ...
          <div class="scope-field scope-lookup-field scope-person-lookup">
            ...
            <div id="preview-suggestions" class="scope-suggestion-anchor"></div>
          </div>
        </div>
        ${splitJsp ? `
  ...
  function renderPersonSuggestions(kind, rows) {
    const action = kind === 'encadrement' ? 'data-enc-add' : (kind === 'preview' ? 'data-add-ex' : 'data-manual-add');
    const query = kind === 'encadrement' ? state.encQuery : (kind === 'preview' ? state.personQuery : state.manualPersonQuery);
  ...
  function renderSaisieRows(rows) {
    ...
    const timeCell = (row) => {
      const effective = L.isEffectiveParticipationStatut ? L.isEffectiveParticipationStatut(row.statut) : row.statut === 'PRESENT';
      if (!effective) return '';
  ...
  function renderAdministration() {
    return `
      <div class="scope-crumb">Administration</div>
      <div class="scope-main">
  ...
  function currentStatComRowsForDisplay() {
    ...
    return L.visibleStatComRows ? L.visibleStatComRows(statComCodes, {
      query: state.formationStatComQuery,
      domainFilter: state.formationStatComDomainFilter,
      stateFilter: state.formationStatComStateFilter,
      sort: state.formationStatComSort
    }) : statComCodes.slice();
  }
  ...
  function renderFormationCatalog() {
    ...
    const motifCatalog = new Map(((participation && participation.motifs) || []).map((row) => [String(row.id || row.value || '').toUpperCase(), row]));
    const form = state.formationDefinitionForm || {};
    const formOpen = Boolean(state.formationFormOpen || form.editDefinitionVersionId);
    const draftMode = form.editDefinitionVersionId ? 'edit' : 'create';
    const versionLabel = (v) => `${escapeHtml(v.version_code || v.versionCode || 'Version')} · ${escapeHtml(L.formatDate(v.valid_from || v.validFrom))} → ${escapeHtml(L.formatDate(v.valid_to || v.validTo))}`;
    const statusLabel = (id) => (statusCatalog.get(String(id || '').toUpperCase()) || {}).label || ({
      PRESENT: 'Présent',
      ABSENT_EXCUSE: 'Excusé',
      ABSENT_NON_EXCUSE: 'Absent',
      DISPENSE: 'Dispensé',
    ...
    const renderPolicyColumn = (kind, title, items, selected, disabledIds = []) => `<section class="scope-policy-column">
      <div class="scope-policy-column-head"><h4>${escapeHtml(title)}</h4>${kind === 'status' ? '<button type="button" class="scope-btn scope-btn-secondary scope-btn-compact" data-referential-add="status">+ Ajouter un statut</button>' : `<button type="button" class="scope-btn scope-btn-secondary scope-btn-compact" data-referential-add="${kind}">+ Ajouter un motif</button>`}</div>
      ${renderChecks(kind, items, selected, disabledIds) || '<p class="scope-empty">Aucun élément actif.</p>'}
      ${renderPolicySummary(title, selected.filter((value) => !(isMulti && value === 'PERMUTATION')), kind === 'status' ? statusLabel : motifLabel)}
    </section>`;
    const renderVersionSummary = (definition, version, selected) => {
      const policy = policyVersions.find((p) => String(p.policy_version_id || p.policyVersionId || '') === String(version.policy_version_id || version.policyVersionId || '')) || {};
      const config = policyConfigOf(policy);
      const statuses = (config.activeStatuses || []).filter((s) => s !== 'NON_RENSEIGNE').map(statusLabel).join(' · ') || '—';
      const excuses = (config.excuseMotifs || []).map(motifLabel).join(' · ') || '—';
    ...
    const rows = definitions.map((definition, index) => {
      ...
      const selected = versions.some((v) => String(v.definition_version_id || v.definitionVersionId || '') === String(state.formationSelectedVersionId || ''));
      ...
      return `<article class="scope-formation-model ${selected ? 'is-open' : ''}" data-zebra="${index % 2 ? 'even' : 'odd'}">
        <header>
        ...
        <div class="scope-formation-meta"><span>Version ${escapeHtml(latest.version_code || latest.versionCode || '—')}</span><span>${escapeHtml(validite)}</span></div>
        ${selected ? detail : ''}
      </article>`;
    }).join('');
    const formHtml = formOpen ? `<div class="scope-card scope-formation-editor" id="formation-editor" tabindex="-1">
            <h2 style="margin-top:0">${draftMode === 'edit' ? 'Modifier la configuration' : 'Créer une formation'}</h2>
            <p class="scope-muted">${draftMode === 'edit' ? 'Modifiez cette version tant qu’aucun événement ne protège son historique.' : 'Renseignez le métier; SCOPE génère les identifiants techniques et choisit la route moteur adaptée.'}</p>
            <div class="scope-report-grid">
              <div class="scope-field"><label>Domaine</label><select id="formation-domain"><option value="">Choisir un domaine</option>${domainOptions}</select></div>
              <div class="scope-field"><label>Nom de la formation</label><input id="formation-label" type="text" value="${escapeHtml(form.label || '')}" placeholder="Formation extincteur"></div>
              <div class="scope-field"><label>Année d’application</label><input id="formation-year" type="number" min="2026" value="${escapeHtml(form.year || '')}"></div>
              <div class="scope-field"><label>Organisation</label><select id="formation-mode">
                <option value="SIMPLE" ${form.modeOrganisation !== 'MULTI_SESSION' ? 'selected' : ''}>Session unique</option>
                <option value="MULTI_SESSION" ${form.modeOrganisation === 'MULTI_SESSION' ? 'selected' : ''}>Plusieurs sessions</option>
              </select></div>
            ...
            <div class="scope-admin-panel scope-policy-builder" style="margin-top:12px">
              ...
              <div class="scope-policy-columns">
                ${renderPolicyColumn('status', 'Statuts disponibles', activeDomain ? statusItems : [], selectedStatuses, isMulti ? ['PRESENT', 'PERMUTATION'] : ['PRESENT'])}
                ${renderPolicyColumn('excuse', 'Motifs d’excuse', activeDomain ? motifItems('EXCUSE', selectedExcuseMotifs) : [], selectedExcuseMotifs)}
                ${renderPolicyColumn('dispense', 'Motifs de dispense', activeDomain ? motifItems('DISPENSE', selectedDispenseMotifs) : [], selectedDispenseMotifs)}
              </div>
              ${isMulti ? '<p class="scope-help">Permutation non disponible pour une formation à plusieurs sessions.</p>' : ''}
            </div>
            <div class="scope-actions">
              <button type="button" class="scope-btn scope-btn-primary" id="formation-create">${draftMode === 'edit' ? 'Enregistrer les modifications' : 'Créer la formation'}</button>
              <button type="button" class="scope-btn scope-btn-secondary" id="formation-cancel-edit">Annuler</button>
            </div>
          </div>` : `<div class="scope-admin-toolbar"><button type="button" class="scope-btn scope-btn-primary" id="formation-open-create">+ Créer une formation</button></div>`;
    const content = !canManage
      ...
      : state.formationCatalogError
        ...
        : `<div class="scope-admin-layout">
          ...
          <div class="scope-card" style="margin-top:12px">
            ...
            <div class="scope-formation-list">${rows || '<div class="scope-empty">Aucun modèle configuré.</div>'}</div>
          </div>
          ${referentialsHtml}
        </div>`;
    return `
  ...
  function qvStatusLabel(value) {
    ...
    return map[String(value || '').toUpperCase()] || String(value || 'À confirmer');
  }

  function qvDayClassLabel(value) {
    const map = {
      PREFERE: 'Préférée',
      AUTORISE: 'Possible',
    ...
    return map[String(value || '').toUpperCase()] || 'Possible';
  }

  function qvTime(value) {
    const text = String(value || '');
    return text.length >= 16 ? text.slice(11, 16) : (/^\d{2}:\d{2}/.test(text) ? text.slice(0, 5) : '');
  }
  ...
  function qvFormatDate(value, empty) {
    const key = qvDateKey(value);
    if (!/^\d{4}-\d{2}-\d{2}$/.test(key)) return empty || 'À proposer';
    return L.formatUiDate(key) || empty || 'À proposer';
  }

  function qvWeekdayLong(value) {
    const date = qvDateKey(value);
    if (!date) return '';
    const label = new Intl.DateTimeFormat('fr-CH', { weekday: 'long', timeZone: 'UTC' }).format(new Date(`${date}T12:00:00Z`));
    return label.charAt(0).toUpperCase() + label.slice(1);
  }

  function qvDateLong(value) {
    const date = qvDateKey(value);
    if (!date) return 'Date à proposer';
    const label = new Intl.DateTimeFormat('fr-CH', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', timeZone: 'UTC' }).format(new Date(`${date}T12:00:00Z`));
    return label.charAt(0).toUpperCase() + label.slice(1);
  }

  function qvMonthLabel(month, year) {
  ...
  function qvIsMultiSession(row) {
    return L.qvIsMultiSessionRow(row);
  }
  ...
  function qvShiftAgendaMonth(monthKey, delta) {
  ...
  function qvLieuRecord(qv, row) {
    const id = row && row.lieuId;
    if (!id) return null;
    return ((qv && qv.lieux) || []).find((lieu) => String(lieu.lieuId) === String(id)) || null;
  }

  function qvOiCode(qv, row) {
    const lieu = qvLieuRecord(qv, row);
    return String((lieu && (lieu.oiCode || lieu.oi_code)) || '').trim();
  }

  function qvCibleLabel(row) {
    return ((row && row.cibleCodes) || []).filter(Boolean).join(', ');
  }

  function qvConvocationAddress(qv, row) {
    const lieu = qvLieuRecord(qv, row);
    if (!lieu) return '';
    const line1 = String(lieu.adresseLigne1 || lieu.adresse_ligne1 || '').trim();
    const line2 = [lieu.npa, lieu.localite].filter(Boolean).join(' ').trim();
  ...
  function qvCursusStepState(qv, row, planned) {
    if (!planned || row.stepRetenu === false) return { tone: 'inactive', label: 'Non prévu' };
    const related = qvActivities(qv).filter((item) => item.cursus && (item.cursus === row.libelle || (item.cursus === 'CI DPS' && row.code === 'CI-DPS')));
  ...
  function qvAgendaDayTitle(date) {
    if (!date || date === 'sans-date') return 'Date à proposer';
    const weekday = qvWeekdayLong(date);
    const day = Number(String(date).slice(8, 10));
    const month = qvMonthLabel(Number(String(date).slice(5, 7)), Number(String(date).slice(0, 4)));
    const year = String(date).slice(0, 4);
    return `${weekday}, ${day} ${month} ${year}`.toUpperCase();
  }

  function qvLieuOptions(qv, selected, includeOther = true) {
    ...
    return `<option value="">Lieu à définir</option>${rows.map((row) => `<option value="${escapeHtml(row.lieuId)}" ${String(row.lieuId) === current ? 'selected' : ''}>${escapeHtml(labelOf(row))}</option>`).join('')}${includeOther ? `<option value="autre" ${autre ? 'selected' : ''}>Autre lieu</option>` : ''}`;
  }

  function qvActivities(qv) {
    return qv.activities || [];
  }

  function qvActivityById(qv, id) {
    return qvActivities(qv).find((row) => String(row.activityId) === String(id)) || null;
  }

  function qvPeriodChoices() {
    return [
  ...
  function qvQuarterChoices() {
    return [
      { value: '2027-T1', label: 'T1 2027' },
      { value: '2027-T2', label: 'T2 2027' },
      { value: '2027-T3', label: 'T3 2027' },
      { value: '2027-T4', label: 'T4 2027' },
      { value: '2028-T1', label: 'T1 2028' }
    ];
  }

  function qvSemesterChoices() {
    return [
      { value: '2027-S1', label: 'S1 2027' },
      { value: '2027-S2', label: 'S2 2027' },
      { value: '2028-S1', label: 'S1 2028' }
    ];
  }

  function qvMonthKeyInQuarter(monthKey, quarter) {
    const match = String(quarter || '').match(/^(\d{4})-T([1-4])$/);
    if (!match || !/^\d{4}-\d{2}$/.test(monthKey)) return false;
    const year = match[1];
    const start = (Number(match[2]) - 1) * 3 + 1;
    const month = Number(monthKey.slice(5, 7));
    return monthKey.slice(0, 4) === year && month >= start && month < start + 3;
  }

  function qvMonthKeyInSemester(monthKey, semester) {
    const match = String(semester || '').match(/^(\d{4})-S([12])$/);
    if (!match || !/^\d{4}-\d{2}$/.test(monthKey)) return false;
    const year = match[1];
    const month = Number(monthKey.slice(5, 7));
    if (monthKey.slice(0, 4) !== year) return false;
    if (match[2] === '1') return month <= 6;
    return month >= 7;
  }

  function qvActivityMatchesPeriod(row, filters) {
    const period = filters.period || 'tous';
    if (period === 'tous') return true;
    const monthKey = row.startsAt ? String(row.startsAt).slice(0, 7) : '';
    if (period === 'mois') {
      const monthFilter = filters.month || 'tous';
      if (!monthFilter || monthFilter === 'tous') return true;
      if (String(monthFilter).length === 2) return monthKey.slice(5, 7) === String(monthFilter).padStart(2, '0');
      return monthKey === monthFilter;
    }
    if (period === 'trimestre') {
      const quarter = filters.quarter || 'tous';
      if (!quarter || quarter === 'tous') return true;
      return qvMonthKeyInQuarter(monthKey, quarter);
    }
    if (period === 'semestre') {
      const semester = filters.semester || 'tous';
      if (!semester || semester === 'tous') return true;
      return qvMonthKeyInSemester(monthKey, semester);
    }
    return true;
  }

  function qvFilteredActivities(qv, options) {
    const opts = options || {};
    const filters = state.quoVadisFilters || {};
    const query = String(filters.q || '').trim().toLowerCase();
    const monthFilter = opts.monthKey || (filters.period === 'mois' ? filters.month : 'tous') || 'tous';
    const rows = qvActivities(qv).filter((row) => {
      ...
      if (!opts.monthKey && !qvActivityMatchesPeriod(row, filters)) return false;
      if (opts.monthKey && monthFilter && monthFilter !== 'tous') {
        if (String(monthFilter).length === 2) {
          if (monthKey.slice(5, 7) !== String(monthFilter).padStart(2, '0')) return false;
        } else if (monthKey !== monthFilter) return false;
      }
      if (filters.lieu && filters.lieu !== 'tous' && qvLieuLabel(row) !== filters.lieu) return false;
      if (filters.oi && filters.oi !== 'tous' && qvOiCode(qv, row) !== filters.oi) return false;
  ...
  function qvActivitySortValue(row, key) {
    if (key === 'date') return qvDateKey(row.startsAt);
    if (key === 'time') return qvTime(row.startsAt) || '';
    if (key === 'domain') return row.domainLabel || row.domain || '';
    if (key === 'title' || key === 'activite') return row.title || '';
    if (key === 'lieu') return row.lieu || '';
    if (key === 'status') return qvStatusLabel(row.status);
    if (key === 'oi') return (row.cibleCodes || []).join(' ');
    return row.title || '';
  }
  ...
  function qvActivityCompareRow(qv, row) {
    return {
      ...
      domain: row && (row.domain || row.domainLabel || ''),
  ...
  function qvFilterOptions(rows, getter) {
    return Array.from(new Set(rows.map(getter).filter(Boolean))).sort((a, b) => String(a).localeCompare(String(b), 'fr'));
  }

  function qvSortHeader(key, label) {
  ...
  function qvFilterBar(qv, options) {
    ...
    const domains = qvFilterOptions(all, (row) => row.domain);
    const cibles = qvFilterOptions(all.flatMap((row) => row.cibleCodes || []), (row) => row);
    ...
    const lieux = qvFilterOptions(all, (row) => qvLieuLabel(row));
  ...
  function qvAgendaFilterBar(qv) {
    const filters = state.quoVadisFilters || {};
    const all = qvActivities(qv);
    const domains = qvFilterOptions(all, (row) => row.domain);
    const families = ['FOBA', 'FOCO', 'FOCA', 'FOSPEC'];
    const ois = qvFilterOptions(all, (row) => qvOiCode(qv, row));
    const specs = qvFilterOptions(all, (row) => row.specialisation);
    const cibles = qvFilterOptions(all.flatMap((row) => row.cibleCodes || []), (row) => row);
    const lieux = qvFilterOptions(all, (row) => qvLieuLabel(row));
    const cursus = qvFilterOptions(all, (row) => row.cursus);
  ...
  function qvActivitesFilterBar(qv) {
    const filters = state.quoVadisFilters || {};
    const all = qvActivities(qv);
    const domains = qvFilterOptions(all, (row) => row.domain);
    const families = ['FOBA', 'FOCO', 'FOCA', 'FOSPEC'];
  ...
  function qvSalleOptions(qv, lieuId, selected) {
    const rooms = qvSallesForLieu(qv, lieuId);
    const current = String(selected || '');
  ...
  function qvResponsableOptions(qv, selected) {
    const current = String(selected || '');
    const rows = (qv && qv.responsableFonctions) || [];
    return `<option value="">À définir</option>${rows.map((row) => `<option value="${escapeHtml(row.code)}" ${row.code === current ? 'selected' : ''}>${escapeHtml(row.libelle)}</option>`).join('')}`;
  }
  ...
  function qvMonthSeparatorLabel(monthKey) {
    if (!/^\d{4}-\d{2}$/.test(monthKey || '')) return 'DATE À PROPOSER';
    return `${qvMonthLabel(Number(monthKey.slice(5, 7)), Number(monthKey.slice(0, 4)))} ${monthKey.slice(0, 4)}`.toUpperCase();
  }

  function qvActiveFiltersMeta() {
    const filters = state.quoVadisFilters || {};
    const sort = state.quoVadisSort || { key: 'date', dir: 'asc' };
    const sortLabel = sort.key === 'date' ? 'Date — horaire — domaine — activité' : `${sort.key} — ${sort.dir === 'desc' ? 'décroissant' : 'croissant'}`;
    const period = filters.period || 'tous';
    const periodLabel = period === 'mois'
      ? (filters.month && filters.month !== 'tous' ? (qvMonthChoices().find((row) => row.value === filters.month) || {}).label || filters.month : 'Mois')
      : period === 'trimestre'
        ? (filters.quarter && filters.quarter !== 'tous' ? (qvQuarterChoices().find((row) => row.value === filters.quarter) || {}).label || filters.quarter : 'Trimestre')
        : period === 'semestre'
          ? (filters.semester && filters.semester !== 'tous' ? (qvSemesterChoices().find((row) => row.value === filters.semester) || {}).label || filters.semester : 'Semestre')
          : 'Tous';
    return {
      search: filters.q || 'Toutes',
      domain: filters.domain && filters.domain !== 'tous' ? filters.domain : 'Tous',
      oi: filters.oi && filters.oi !== 'tous' ? filters.oi : 'Tous',
      status: filters.status && filters.status !== 'tous' ? qvAgendaState({ status: filters.status }).label : 'Tous',
      month: periodLabel,
      sort: sortLabel
    };
  }

  function qvExportRows(rows, qv) {
    const data = qv || quoVadisData();
    return rows.map((row) => {
      const monthKey = row.startsAt ? String(row.startsAt).slice(0, 7) : '';
      return {
        ...
        horaire: [qvTime(row.startsAt), qvTime(row.endsAt)].filter(Boolean).join(' – ') || 'À définir',
        domaine: row.domainLabel || row.domain || '',
        oi: qvOiCode(data, row) || '—',
        publicCible: qvCibleLabel(row) || '—',
        activite: row.title || '',
        specCursus: row.specCursus || [row.specialisation, row.cursus].filter(Boolean).join(' · '),
        statcom: row.statcomCode || '',
        lieu: qvLieuLabel(row),
        adresse: qvConvocationAddress(data, row).replace(/\n/g, ', '),
        salleTheorie: row.salleTheorie || '',
        responsable: row.responsable || '',
        etat: qvAgendaState(row).label,
  ...
  function qvRenderMiniMonth(month, today) {
    ...
    return `<article class="qv-year-month qv-mini-month">
        ...
        <div class="qv-mini-cal">
          <span class="qv-mini-week-head" aria-hidden="true">S</span>
          ${['L', 'M', 'M', 'J', 'V', 'S', 'D'].map((d) => `<span class="qv-mini-head">${d}</span>`).join('')}
          ${(() => {
            const rows = [];
            for (let start = 0; start < month.cells.length; start += 7) {
              const slice = month.cells.slice(start, start + 7);
              const dated = slice.find((cell) => cell && cell.date);
              const week = dated ? L.qvIsoWeek(dated.date) : '';
              rows.push(`<span class="qv-mini-week">${week ? escapeHtml(String(week)) : ''}</span>`);
              slice.forEach((cell, offset) => {
                const index = start + offset;
                if (!cell) {
                  rows.push('<span class="qv-mini-empty"></span>');
                  return;
                }
  ...
  function renderQuoVadisAgenda(qv) {
    const monthKey = qvAgendaMonthKey(qv);
    const [year, month] = monthKey.split('-').map(Number);
    const prev = qvShiftAgendaMonth(monthKey, -1);
    const next = qvShiftAgendaMonth(monthKey, 1);
    const rows = qvFilteredActivities(qv, { monthKey });
    const groups = [];
    rows.forEach((row) => {
      const date = qvDateKey(row.startsAt) || 'sans-date';
      const last = groups[groups.length - 1];
      if (!last || last.date !== date) groups.push({ date, rows: [row] });
      else last.rows.push(row);
    });
    const calendar = qvCalendarIndex(qv);
    const eventCount = rows.length;
    const dayCount = groups.filter((group) => group.date !== 'sans-date').length;
    const eventWord = eventCount <= 1 ? 'événement' : 'événements';
    const dayWord = dayCount <= 1 ? 'journée' : 'journées';
    const body = groups.map((group) => {
      const marks = L.qvCalendarMarksForDate(calendar, group.date);
      const holidayRow = ((calendar[group.date] || []).find((row) => qvCalendarKind(row) === 'FERIE')) || null;
      const dayClass = marks.holiday ? 'is-holiday' : marks.vacation ? 'is-vacation' : 'is-normal';
      const holidayNote = marks.holiday && holidayRow && holidayRow.libelle
        ? `<span class="qv-agenda-holiday-label">Jour férié — ${escapeHtml(holidayRow.libelle)}</span>`
        : '';
      const week = group.date !== 'sans-date' ? L.qvIsoWeek(group.date) : 0;
      return `<tr class="qv-agenda-day qv-agenda-date-row ${dayClass}" id="${escapeHtml(L.qvAgendaDayAnchorId(group.date))}"><th colspan="9"><div class="qv-agenda-day-bar"><span class="qv-agenda-day-main">${escapeHtml(qvAgendaDayTitle(group.date))}${holidayNote}</span>${week ? `<span class="qv-agenda-week">Semaine ${escapeHtml(String(week))}</span>` : ''}</div></th></tr>${group.rows.map((row) => {
        const address = qvConvocationAddress(qv, row);
        const cancelled = String(row.status || '').toUpperCase() === 'ANNULE';
        return `<tr class="qv-agenda-row${cancelled ? ' is-cancelled' : ''}" data-qv-open="${qvHref('activites', { id: row.activityId, from: 'agenda' })}">
          ...
          <td>${escapeHtml(qvOiCode(qv, row) || '—')}</td>
          <td>${escapeHtml(qvCibleLabel(row) || '—')}</td>
          <td><a class="scope-events-libelle qv-agenda-title" href="${qvHref('activites', { id: row.activityId, from: 'agenda' })}">${escapeHtml(row.title)}</a></td>
    ...
    return `<section class="scope-card qv-agenda-view">
      ...
      <div class="scope-table-wrap">
        <table class="scope-table qv-agenda-table">
          <colgroup>
            <col class="qv-col-time"><col class="qv-col-domain"><col class="qv-col-oi"><col class="qv-col-cible"><col class="qv-col-title"><col class="qv-col-statcom"><col class="qv-col-lieu"><col class="qv-col-address"><col class="qv-col-state">
          </colgroup>
          <thead><tr><th>Horaire</th><th>Domaine</th><th>OI</th><th>Public cible</th><th>Événement</th><th>Stat.Com</th><th>Lieu</th><th>Adresse convocation</th><th>État</th></tr></thead>
          <tbody>${body || '<tr><td colspan="9"><div class="scope-empty">Aucune activité ne correspond aux filtres pour ce mois.</div></td></tr>'}</tbody>
        </table>
      </div>
      <p class="qv-agenda-total">${escapeHtml(String(eventCount))} ${eventWord} au total sur ce mois</p>
      <div class="qv-agenda-notes">
  ...
  function renderQuoVadisActivites(qv) {
    ...
    const monthCounts = {};
    rows.forEach((row) => {
      const key = row.startsAt ? String(row.startsAt).slice(0, 7) : 'sans-date';
      monthCounts[key] = (monthCounts[key] || 0) + 1;
    });
    const body = rows.length ? rows.map((row) => {
      const monthKey = row.startsAt ? String(row.startsAt).slice(0, 7) : 'sans-date';
      const href = qvHref('activites', { id: row.activityId, from: 'activites' });
      const dayClass = qvActivityCalendarClass(qv, row);
      let separator = '';
      ...
      return `${separator}<tr class="qv-activites-row ${dayClass}" data-qv-open="${href}">
            ...
            <td>${escapeHtml(qvOiCode(qv, row) || '—')}</td>
            <td>${escapeHtml(qvCibleLabel(row) || '—')}</td>
            <td class="qv-activites-title"><a class="scope-events-libelle" href="${href}">${escapeHtml(row.title)}</a></td>
      ...
      <div class="scope-table-wrap qv-activites-wrap">
        <table class="scope-table qv-activities-table">
          ...
          <thead><tr>
            ...
            <th>Responsable</th>
            <th>État</th>
            <th class="qv-activites-action"><span class="scope-visually-hidden">Fiche</span></th>
          </tr></thead>
          <tbody>${body}</tbody>
        </table>
      </div>
  ...
  function renderQuoVadisActivite(qv) {
    const id = route().qvActivityId;
    const from = route().qvFrom || 'activites';
    const row = qvActivityById(qv, id);
    if (!row) return `<section class="scope-card"><p class="scope-empty">Cette activité n’est pas disponible.</p>${contextReturnHtml(qvHref(from === 'agenda-annuel' ? 'agenda-annuel' : from, { jour: route().qvJour }), 'Retour')}</section>`;
    ...
    return `<section class="scope-card qv-activity-detail">
      <div class="scope-section-head">
        ...
        ${contextReturnHtml(backHref, backLabel)}
      </div>
      <div class="scope-grid scope-grid-2">
        <section class="scope-form-section"><h3>Activité</h3><dl class="qv-definition-list">
          <dt>Intitulé</dt><dd>${escapeHtml(row.title)}</dd>
          <dt>Domaine</dt><dd>${escapeHtml(row.domainLabel || row.domain || '—')}</dd>
          <dt>Public cible</dt><dd>${escapeHtml(qvCibleLabel(row) || '—')}</dd>
          <dt>OI</dt><dd>${escapeHtml(qvOiCode(qv, row) || '—')}</dd>
        ...
        <section class="scope-form-section"><h3>Planification proposée</h3><dl class="qv-definition-list">
          ...
          <dt>Jour</dt><dd>${escapeHtml(row.weekday || qvWeekdayLong(row.startsAt) || '—')}</dd>
          <dt>Horaire début</dt><dd>${escapeHtml(qvTime(row.startsAt) || 'À définir')}</dd>
          <dt>Horaire fin</dt><dd>${escapeHtml(qvTime(row.endsAt) || 'À définir')}</dd>
          <dt>Lieu</dt><dd>${escapeHtml(qvLieuLabel(row))}</dd>
          <dt>Salle théorie</dt><dd><select class="qv-activity-salle" id="qv-activity-salle" data-qv-activity-field="salleTheorieId">${qvSalleOptions(qv, row.lieuId, row.salleTheorieId)}</select></dd>
          <dt>Responsable</dt><dd><select class="qv-activity-responsable" id="qv-activity-responsable" data-qv-activity-field="responsableFonctionCode">${qvResponsableOptions(qv, row.responsableFonctionCode)}</select></dd>
          <dt>État</dt><dd>${qvAgendaStateBadge(row)}</dd>
        </dl></section>
      </div>
      ...
      <section class="scope-form-section">
        ...
        <ul class="qv-reasons">${(row.reasons || []).map((text) => `<li>${escapeHtml(text)}</li>`).join('') || '<li>Les règles et contraintes connues seront appliquées au prochain recalcul.</li>'}</ul>
      </section>
      <section class="scope-form-section">
      ...
      <section class="scope-form-section">
        ...
        <div class="scope-table-wrap"><table class="scope-table"><thead><tr><th>Date</th><th>Jour</th><th>Horaire</th><th>Lieu</th><th>Préférence</th><th>Justification</th><th>Contraintes</th><th>Action</th></tr></thead><tbody>${proposals.map((p) => {
          const conflict = p.conflictSummary || {};
          const weekday = p.startsAt ? qvWeekdayLong(p.startsAt) : '—';
          return `<tr>
            <td>${escapeHtml(p.startsAt ? qvFormatDate(p.startsAt) : 'À proposer')}</td>
            <td>${escapeHtml(weekday)}</td>
            <td>${escapeHtml([qvTime(p.startsAt), qvTime(p.endsAt)].filter(Boolean).join(' – '))}</td>
            <td><select class="qv-proposal-lieu" data-qv-proposal-lieu="${escapeHtml(p.proposalId)}">${qvLieuOptions(qv, p.lieuId || row.lieuId)}</select></td>
            <td>${escapeHtml(p.dayClassLabel || qvDayClassLabel(p.dayClass) || '—')}</td>
            <td>${escapeHtml((p.reasons || []).slice(0, 2).join(' ') || 'Proposition du moteur de préparation.')}</td>
            <td>${escapeHtml([conflict.conflict ? 'Conflit' : '', conflict.requiresDerogation ? 'Dérogation à motiver' : '', conflict.prerequisite === 'A_VERIFIER' ? 'Prérequis à vérifier' : '', conflict.astreinte === 'A_VERIFIER' ? 'Astreinte à vérifier' : ''].filter(Boolean).join(' · ') || 'Aucune réserve majeure')}</td>
            <td><button type="button" class="scope-btn scope-btn-compact" data-qv-retain="${escapeHtml(p.proposalId)}">${p.status === 'RETENU' ? 'Proposition retenue' : 'Retenir cette proposition'}</button></td>
          </tr>`;
        }).join('') || '<tr><td colspan="8"><div class="scope-empty">Aucune proposition disponible.</div></td></tr>'}</tbody></table></div>
      </section>
      <section class="scope-form-section">
        <h3>Référence 2026</h3>
        <p class="scope-muted">Aide à la décision uniquement. Les règles 2027 restent prioritaires. Aucune date 2026 n’est recopiée automatiquement.</p>
        ${state.quoVadisHistoryError ? `<p class="scope-state-error">${escapeHtml(state.quoVadisHistoryError)}</p>` : ''}
        ${!state.quoVadisHistoryReady ? '<p class="scope-empty">Recherche d’une référence comparable…</p>' : (history.length ? `<div class="scope-table-wrap"><table class="scope-table"><thead><tr><th>Activité comparable</th><th>Date</th><th>Jour</th><th>Horaire</th><th>Lieu</th><th>Organisation / OI</th><th>Fiabilité</th></tr></thead><tbody>${history.map((ref) => `<tr>
          <td>${escapeHtml(ref.title)}</td>
          <td>${escapeHtml(qvFormatDate(ref.date, '—'))}</td>
          <td>${escapeHtml(ref.weekday || '—')}</td>
          <td>${escapeHtml([ref.startsAt, ref.endsAt].filter(Boolean).join(' – ') || '—')}</td>
          <td>${escapeHtml(ref.lieu || 'Lieu à définir')}</td>
          <td>${escapeHtml([ref.organisation, ref.sousDomaine].filter(Boolean).join(' · ') || '—')}</td>
          <td>${escapeHtml(ref.confidence || (ref.uncertain ? 'Correspondance à confirmer' : 'Correspondance probable'))}</td>
        </tr>`).join('')}</tbody></table></div>` : '<p class="scope-empty">Aucune référence 2026 comparable.</p>')}
      </section>
    </section>`;
  }

  function qvArbitrerFilterBar(qv, activities, mode) {
    const filters = state.quoVadisFilters || {};
    const rows = activities || qvActivities(qv);
    const domains = L.sortByScopeDomainOrder(qvFilterOptions(rows, (row) => row.domain), (value) => value);
    const ois = qvFilterOptions(rows, (row) => qvOiCode(qv, row));
    const states = qvFilterOptions(rows, (row) => row.status);
    const families = ['FOBA', 'FOCO', 'FOCA', 'FOSPEC'];
    const current = mode === 'detaillee' ? 'detaillee' : 'groupes';
    const field = (id, label, optionsHtml) => `<div class="scope-field"><label for="${id}">${escapeHtml(label)}</label><select id="${id}">${optionsHtml}</select></div>`;
    return `<div class="qv-agenda-filters qv-arbitrer-filters">
      <div class="scope-field qv-filter-search"><label for="qv-filter-q">Recherche</label><div class="qv-filter-search-control"><span class="qv-filter-search-icon">${qvCockpitIcon('search')}</span><input id="qv-filter-q" type="search" value="${escapeHtml(filters.q || '')}" placeholder="Rechercher une activité, un code cours, un lieu…"></div></div>
      ${field('qv-filter-domain', 'Domaine', `<option value="tous">Tous</option>${domains.map((v) => `<option value="${escapeHtml(v)}" ${filters.domain === v ? 'selected' : ''}>${escapeHtml(v)}</option>`).join('')}`)}
      ${field('qv-filter-family', 'Famille', `<option value="tous">Toutes</option>${families.map((v) => `<option value="${escapeHtml(v)}" ${filters.family === v ? 'selected' : ''}>${escapeHtml(v)}</option>`).join('')}`)}
      ${field('qv-filter-oi', 'OI', `<option value="tous">Tous</option>${ois.map((v) => `<option value="${escapeHtml(v)}" ${filters.oi === v ? 'selected' : ''}>${escapeHtml(v)}</option>`).join('')}`)}
      ${field('qv-filter-status', 'État', `<option value="tous">Tous</option>${states.map((v) => `<option value="${escapeHtml(v)}" ${filters.status === v ? 'selected' : ''}>${escapeHtml(qvAgendaState({ status: v }).label)}</option>`).join('')}`)}
      <button type="button" class="qv-arbitrer-reset scope-text-action" id="qv-filter-reset">Réinitialiser</button>
      <div class="qv-period-seg qv-arbitrer-seg" role="group" aria-label="Mode d’affichage">
        <button type="button" class="qv-period-seg-btn${current === 'groupes' ? ' is-active' : ''}" data-qv-arbitrer-mode="groupes" aria-pressed="${current === 'groupes' ? 'true' : 'false'}">Vue par groupes</button>
        <button type="button" class="qv-period-seg-btn${current === 'detaillee' ? ' is-active' : ''}" data-qv-arbitrer-mode="detaillee" aria-pressed="${current === 'detaillee' ? 'true' : 'false'}">Vue détaillée</button>
      </div>
    </div>`;
  }

  function qvArbitrerGroupHaystack(qv, group) {
    const parts = [
      group.title,
      group.storedTitle,
      group.domain,
      ...
      ...(group.proposals || []).flatMap((proposal) => [
        proposal.lieu,
        (proposal.conflictSummary && proposal.conflictSummary.sessionLabel) || '',
  ...
  function qvArbitrerGroupMatches(qv, group, filters) {
    ...
    if (filters.type === 'multi' && !group.multi) return false;
    if (filters.status && filters.status !== 'tous' && !(group.rows || []).some((row) => row.status === filters.status)) return false;
    if (filters.attention && !group.attention) return false;
    return true;
  }

  function qvArbitrerOpenMap(domainGroups) {
    const first = L.qvInitialOpenDomain(domainGroups);
    if (!state.quoVadisArbitrerTouched) return first ? { [first]: true } : {};
    const current = Object.assign({}, state.quoVadisArbitrerOpenDomains || {});
    const visibleOpen = (domainGroups || []).some((row) => current[row.code]);
    if (!visibleOpen && first) current[first] = true;
    return current;
  }
  ...
  function qvArbitrerSortedGroups(groups) {
    return L.sortRows(groups, state.quoVadisArbitrerSort, L.qvArbitrerSortColumns());
  }

  function qvArbitrerEtat(group) {
    const current = L.qvArbitrageState(group);
    return scopeStateHtml(current.tone, current.label);
  }

  function qvArbitrerAction(group) {
    const href = qvHref('activites', { id: group.activityId, from: 'a-arbitrer' });
  ...
  function qvDomainPeriodLabel(groups) {
    const dates = (groups || []).flatMap((group) => [group.firstStartsAt]
      ...
      .concat((group.rows || []).map((row) => row && row.startsAt)));
    return L.qvArbitragePeriodLabel(dates, true) || '—';
  }

  function qvDomainEtat(groups) {
    const current = L.qvArbitrageState({
      validated: (groups || []).length > 0 && (groups || []).every((group) => group.validated),
      attention: (groups || []).some((group) => group.attention),
      needsArbitration: (groups || []).some((group) => group.needsArbitration),
      status: (((groups || []).find((group) => group.needsArbitration) || (groups || [])[0]) || {}).status
    });
    return scopeStateHtml(current.tone, current.label);
  }

  function qvRenderArbitrerKpis(kpis) {
    const items = [
      [kpis.aArbitrer, 'activités à arbitrer'],
      [kpis.propositions, 'propositions de dates'],
      [kpis.cursusMulti, 'cursus / multi-sessions'],
      [kpis.attention, 'points d’attention'],
      [kpis.validees, Number(kpis.validees) === 1 ? 'déjà validée' : 'déjà validées']
    ];
  ...
  function qvArbitrerCountLabel(count, singular, plural) {
    const value = Number(count || 0);
    return `${value} ${value === 1 ? singular : plural}`;
  }

  function qvRenderArbitrerDomainHead(domain, groups) {
    const proposalCount = (groups || []).reduce((sum, group) => sum + Number(group.proposalCount || 0), 0);
    return `<div class="qv-arbitrer-domain-head">
      <div class="qv-arbitrer-domain-head-main">
        <span class="qv-arbitrer-chevron" aria-hidden="true"></span>
        <strong class="qv-arbitrer-domain">${escapeHtml(domain.code)}</strong>
        <span class="qv-arbitrer-domain-label">${escapeHtml(domain.label || '')}</span>
      </div>
      <div class="qv-arbitrer-domain-head-meta">
        <span>${escapeHtml(qvArbitrerCountLabel(groups.length, 'activité', 'activités'))}</span>
        <span>${escapeHtml(qvArbitrerCountLabel(proposalCount, 'proposition', 'propositions'))}</span>
        <span class="qv-arbitrer-domain-head-period">${qvArbitrerPeriodHtml(qvDomainPeriodLabel(groups))}</span>
      </div>
    </div>`;
  }

  function qvRenderArbitrerGroupTable(groups) {
    return `<div class="scope-table-wrap qv-arbitrer-table-wrap"><table class="scope-table qv-arbitrer-table qv-arbitrer-activity-table qv-arbitrer-detail-table">
      <thead><tr>
        ${qvArbitrerSortHeader('codeCours', 'Code cours')}
        ${qvArbitrerSortHeader('title', 'Activité')}
        ${qvArbitrerSortHeader('statCom', 'Stat.Com')}
        ${qvArbitrerSortHeader('date', 'Date')}
        ${qvArbitrerSortHeader('horaire', 'Horaire')}
        ${qvArbitrerSortHeader('oi', 'OI')}
        ${qvArbitrerSortHeader('cible', 'Public cible')}
      ...
      <tbody>${qvArbitrerSortedGroups(groups).map((group) => `<tr>
        ...
        <td class="qv-col-resp">${escapeHtml(qvNeutralCell(group.responsable))}</td>
      ...
      </tr>`).join('') || `<tr><td colspan="11"><div class="scope-empty">Aucune activité comparable.</div></td></tr>`}</tbody>
    </table></div>`;
  }

  function qvRenderArbitrerDomainsTable(domainGroups, openMap) {
    return `<div class="scope-table-wrap qv-arbitrer-table-wrap"><table class="scope-table qv-arbitrer-table qv-arbitrer-group-table">
      <thead><tr>
        <th>Domaine</th>
        <th>Description</th>
        <th class="qv-col-num">Activités</th>
        <th class="qv-col-num">Propositions</th>
      ...
      ${domainGroups.map((domain) => {
        ...
        const proposalCount = groups.reduce((sum, group) => sum + Number(group.proposalCount || 0), 0);
        return `<tbody class="${open ? 'is-open' : ''}">
        <tr class="qv-arbitrer-domain-row${open ? ' is-open' : ''}">
          ...
          <td class="qv-col-desc"><span class="qv-arbitrer-domain-label">${escapeHtml(domain.label || '')}</span></td>
          <td class="qv-col-num">${escapeHtml(String(groups.length))}</td>
          <td class="qv-col-num">${escapeHtml(String(proposalCount))}</td>
          <td class="qv-col-period">${qvArbitrerPeriodHtml(qvDomainPeriodLabel(groups))}</td>
  ...
  function qvRenderArbitrerDetail(qv, domainGroups) {
    return domainGroups.map((domain) => {
      const groups = qvArbitrerSortedGroups((domain.groups || []).map((group) => qvArbitrerEnrichGroup(qv, group)));
      return `<section class="qv-arbitrer-domain-block scope-table-hierarchy">
      ${qvRenderArbitrerDomainHead(domain, groups)}
      ${qvRenderArbitrerGroupTable(groups)}
    </section>`;
    }).join('');
  }
  ...
  function renderQuoVadisCursus(qv) {
    const year = Number((qv.programme || {}).annee || 2027);
    ...
    const steps = (qv.cursus || []).filter((row) => row.code === selectedCode && row.stepId && row.active !== false).slice().sort((a, b) => Number(a.ordre || 0) - Number(b.ordre || 0));
    const editing = state.quoVadisCursusEdit === selectedCode;
    const stepEdit = steps.find((row) => row.stepId === state.quoVadisModuleEdit);
    const scheduleStep = steps.find((row) => row.stepId === state.quoVadisScheduleStepId);
    const targetOptions = (domain, value) => sharedOptionsHtml([{ value: '', label: 'Aucun public particulier' }].concat((L.eventCiblesForForm ? L.eventCiblesForForm(domain, qv.cibles || [], value ? [value] : []) : []).map((row) => ({ value: row.cibleId, label: row.libelle || row.niveauCode }))), value || '');
    ...
    return `<div class="qv-cursus-view qv-pilot-view">
      <section class="scope-card qv-cursus-nav">
        ...
        ${state.quoVadisNewCursus ? `<form id="qv-cursus-create" class="qv-cursus-create scope-field"><label for="qv-cursus-name">Nom du cursus</label><input id="qv-cursus-name" type="text" required maxlength="160" autocomplete="off"><label for="qv-cursus-code">Code (facultatif)</label><input id="qv-cursus-code" type="text" maxlength="80"><label for="qv-cursus-domain">Domaine</label><select id="qv-cursus-domain" required>${domainTaxonomySelectHtml('', { emptyLabel: 'Sélectionner…' })}</select><label for="qv-cursus-target">Public cible</label><select id="qv-cursus-target"><option value="">Aucun public particulier</option></select><label for="qv-cursus-duration">Durée (mois)</label><input id="qv-cursus-duration" type="number" min="1" max="240" required value="12"><label for="qv-cursus-description">Description</label><textarea id="qv-cursus-description" rows="2"></textarea><div class="qv-cursus-create-actions"><button type="submit" class="scope-btn qv-soft-btn">Créer</button><button type="button" class="scope-btn qv-soft-btn" id="qv-cursus-cancel">Annuler</button></div></form>` : ''}
        ...
        <ul class="qv-cursus-list">${visible.map((row) => `<li>
          <button type="button" class="${row.code === selectedCode ? 'is-active' : ''}" data-qv-cursus-select="${escapeHtml(row.code)}">
            ...
            ${scopeStateHtml(row.retenu ? 'positive' : 'inactive', row.retenu ? `Prévu en ${year}` : `Non prévu en ${year}`)}
          </button>
      ...
      <section class="scope-card qv-cursus-detail">
        ${selected ? `<div class="scope-section-head">
          <div>
            ...
            ${scopeStateHtml(planned ? 'positive' : 'inactive', planned ? `Prévu en ${year}` : `Non prévu en ${year}`)}
            ${selected.justification ? `<p class="qv-pilot-kicker">${escapeHtml(selected.justification)}</p>` : ''}
          </div>
          ${hasScopePermission('references:manage') ? `<div>${qvSoftBtn('qv-cursus-edit', 'Modifier le référentiel')}${qvSoftBtn('qv-cursus-delete', 'Archiver / supprimer')}</div>` : ''}
        </div>
        <div class="qv-local-feedback" id="qv-cursus-feedback" role="status">${escapeHtml(state.quoVadisCursusFeedback || '')}</div>
        ${editing ? `<form id="qv-cursus-edit-form" class="qv-reference-form scope-form-section">
          <div class="scope-form-two"><div class="scope-field"><label for="qv-cursus-edit-name">Nom</label><input id="qv-cursus-edit-name" required value="${escapeHtml(selected.libelle)}"></div><div class="scope-field"><label for="qv-cursus-edit-duration">Durée (mois)</label><input id="qv-cursus-edit-duration" type="number" min="1" max="240" required value="${escapeHtml(selected.durationMonths || '')}"></div></div>
          <div class="scope-form-two"><div class="scope-field"><label for="qv-cursus-edit-domain">Domaine</label><select id="qv-cursus-edit-domain">${domainTaxonomySelectHtml(selected.domainCode || '', { emptyLabel: 'Sélectionner…' })}</select></div><div class="scope-field"><label for="qv-cursus-edit-target">Public cible</label><select id="qv-cursus-edit-target">${targetOptions(selected.domainCode, selected.cibleId)}</select></div></div>
          <div class="scope-form-two"><div class="scope-field"><label for="qv-cursus-edit-description">Description</label><textarea id="qv-cursus-edit-description" rows="2">${escapeHtml(selected.description || '')}</textarea></div><div class="scope-field"><label for="qv-cursus-edit-status">État</label><select id="qv-cursus-edit-status"><option value="ACTIF" ${selected.statut === 'ACTIF' ? 'selected' : ''}>Actif</option><option value="INACTIF" ${selected.statut === 'INACTIF' ? 'selected' : ''}>Inactif</option><option value="ARCHIVE" ${selected.statut === 'ARCHIVE' ? 'selected' : ''}>Archivé</option></select></div></div>
          <button type="submit" class="scope-btn qv-soft-btn">Enregistrer</button><button type="button" class="scope-btn" id="qv-cursus-edit-cancel">Annuler</button>
        </form>` : ''}
        <section class="scope-form-section">
          <h3>1. Planning ${year}</h3>
          <div class="qv-cursus-plan">
            <label><input type="radio" name="qv-cursus-plan" data-qv-cursus-plan="${escapeHtml(selected.code)}" value="1" ${planned ? 'checked' : ''} ${manageable ? '' : 'disabled'}> Prévu en ${year}</label>
            <label><input type="radio" name="qv-cursus-plan" data-qv-cursus-plan="${escapeHtml(selected.code)}" value="0" ${planned ? '' : 'checked'} ${manageable ? '' : 'disabled'}> Non prévu en ${year}</label>
          </div>
          ${qvPilotHintHtml(`Si un cursus n’est pas planifié en ${year}, ses modules sont décochés et deviennent indisponibles pour cette année.`)}
        </section>
        <section class="scope-form-section qv-cursus-modules">
          <h3>2. Modules du cursus <small>(${steps.length})</small></h3>
          <div class="scope-table-wrap qv-pilot-table-wrap"><table class="scope-table qv-pilot-table"><thead><tr><th>Module</th><th>Libellé</th><th>Année</th><th>Période proposée</th><th>État</th><th>Inclus ${year}</th><th>Gestion</th></tr></thead>
            <tbody>${steps.map((row, index) => {
              ...
              <td>${escapeHtml((row.annualSchedule || {}).date || qvCursusStepPeriod(qv, row))}</td>
              <td>${scopeStateHtml(visual.tone, visual.label)}</td>
              <td><input type="checkbox" data-qv-cursus-step="${escapeHtml(row.stepId || '')}" ${included ? 'checked' : ''} ${planned && manageable ? '' : 'disabled'}></td>
              <td>${hasScopePermission('references:manage') ? `<button type="button" class="scope-text-action" data-qv-module-edit="${escapeHtml(row.stepId)}">Modifier</button><button type="button" class="scope-text-action" data-qv-module-delete="${escapeHtml(row.stepId)}">Archiver</button><button type="button" class="scope-text-action" data-qv-module-move="${escapeHtml(row.stepId)}" data-direction="-1" ${index === 0 ? 'disabled' : ''} title="Monter">↑</button><button type="button" class="scope-text-action" data-qv-module-move="${escapeHtml(row.stepId)}" data-direction="1" ${index === steps.length - 1 ? 'disabled' : ''} title="Descendre">↓</button>${planned && included && selected.code !== 'CI-DPS' ? `<button type="button" class="scope-text-action" data-qv-step-schedule="${escapeHtml(row.stepId)}">Planifier</button>` : ''}` : ''}</td>
            </tr>`;
            }).join('') || '<tr><td colspan="7"><p class="qv-pilot-empty">Aucun module pour ce cursus.</p></td></tr>'}</tbody>
          </table></div>
          ${scheduleStep && planned && scheduleStep.stepRetenu ? `<form id="qv-step-schedule-form" class="qv-reference-form" data-step-id="${escapeHtml(scheduleStep.stepId)}"><h4>Occurrence ${year} · ${escapeHtml(scheduleStep.stepLabel)}</h4>
            <div class="scope-form-two"><div class="scope-field"><label for="qv-step-date">Date</label><input id="qv-step-date" type="date" required value="${escapeHtml((scheduleStep.annualSchedule || {}).date || '')}"></div><div class="scope-field"><label for="qv-step-duration">Durée (minutes)</label><input id="qv-step-duration" type="number" min="1" max="10080" required value="${escapeHtml((scheduleStep.annualSchedule || {}).durationMinutes || scheduleStep.durationMinutes || '')}"></div></div>
            <div class="scope-form-two"><div class="scope-field"><label for="qv-step-start">Début</label><input id="qv-step-start" type="time" required value="${escapeHtml((scheduleStep.annualSchedule || {}).start || scheduleStep.usualStartTime || '')}"></div><div class="scope-field"><label for="qv-step-end">Fin</label><input id="qv-step-end" type="time" required value="${escapeHtml((scheduleStep.annualSchedule || {}).end || scheduleStep.usualEndTime || '')}"></div></div>
            <div class="scope-field"><label for="qv-step-place">Lieu</label><select id="qv-step-place">${qvLieuOptions(qv, (scheduleStep.annualSchedule || {}).lieuId || scheduleStep.lieuId || '', false)}</select></div>
            <button type="submit" class="scope-btn qv-soft-btn">Enregistrer l’occurrence</button><button type="button" id="qv-step-schedule-cancel" class="scope-btn">Annuler</button>
          </form>` : ''}
          ${hasScopePermission('references:manage') && manageable ? `${qvSoftBtn('qv-module-new', '＋ Ajouter un module')}${(state.quoVadisModuleNew || stepEdit) ? `<form id="qv-module-form" class="qv-reference-form" data-step-id="${escapeHtml(stepEdit && stepEdit.stepId || '')}">
            <div class="scope-form-two"><div class="scope-field"><label for="qv-module-name">Libellé</label><input id="qv-module-name" required value="${escapeHtml(stepEdit && stepEdit.stepLabel || '')}"></div><div class="scope-field"><label for="qv-module-year">Année du cursus</label><input id="qv-module-year" type="number" min="1" max="30" required value="${escapeHtml(stepEdit && stepEdit.logicalYear || 1)}"></div></div>
            <div class="scope-form-two"><div class="scope-field"><label for="qv-module-minutes">Durée (minutes)</label><input id="qv-module-minutes" type="number" min="1" max="10080" required value="${escapeHtml(stepEdit && stepEdit.durationMinutes || '')}"></div><div class="scope-field"><label for="qv-module-month">Mois habituel</label><input id="qv-module-month" type="number" min="1" max="12" value="${escapeHtml(stepEdit && stepEdit.preferredMonth || '')}"></div></div>
            <div class="scope-form-two"><div class="scope-field"><label for="qv-module-start">Début habituel</label><input id="qv-module-start" type="time" value="${escapeHtml(stepEdit && stepEdit.usualStartTime || '')}"></div><div class="scope-field"><label for="qv-module-end">Fin habituelle</label><input id="qv-module-end" type="time" value="${escapeHtml(stepEdit && stepEdit.usualEndTime || '')}"></div></div>
            <div class="scope-form-two"><div class="scope-field"><label for="qv-module-place">Lieu</label><select id="qv-module-place">${qvLieuOptions(qv, stepEdit && stepEdit.lieuId || '', false)}</select></div><div class="scope-field"><label for="qv-module-target">Public cible</label><select id="qv-module-target">${targetOptions(selected.domainCode, stepEdit && stepEdit.cibleId)}</select></div></div>
            <button type="submit" class="scope-btn qv-soft-btn">Enregistrer</button><button type="button" id="qv-module-cancel" class="scope-btn">Annuler</button>
          </form>` : ''}` : ''}
        </section>
  ...
  function renderQuoVadisRegles(qv) {
    ...
    const editor = state.quoVadisRuleEditor;
    const activeRule = editor && editor !== 'new' ? rules.find((row) => row.ruleId === editor) : null;
    ...
    return `<section class="scope-card qv-pilot-view qv-regles-view">
      <div class="scope-section-head">
        ...
        ${hasScopePermission('references:manage') ? qvSoftBtn('qv-rules-edit', '＋ Nouvelle règle') : ''}
      </div>
  ...
  function qvFieldError(name) {
    const message = (state.quoVadisFutureErrors || {})[name];
    return message ? `<small class="qv-field-error">${escapeHtml(message)}</small>` : '';
  }
  ...
  function renderQuoVadisDatesConnues(qv) {
    ...
    const oiOptions = [{ value: '', label: 'Non précisé' }].concat((L.eventCiblesForForm ? L.eventCiblesForForm(domain, qv.cibles || [], []) : [])
      .map((row) => ({ value: row.niveauCode, label: row.libelle || (L.niveauAffiche ? L.niveauAffiche(domain, row.niveauCode) : row.niveauCode) })));
      ...
      <section class="scope-card qv-dates-form">
        ...
        <section class="scope-form-section is-wide">
          ...
          <div class="scope-form-two qv-form-follow">
            ...
            <div class="scope-field"><label for="qv-future-cible">OI OU PUBLIC CIBLE</label><select id="qv-future-cible">${sharedOptionsHtml(oiOptions, selectedOi)}</select></div>
          </div>
    ...
    return `<section class="annual-catalogue" aria-labelledby="annual-catalog-title">
      ...
      <div class="annual-table-wrap"><table class="annual-table"><thead><tr><th>Domaine</th><th>Activité</th><th>Besoin ${escapeHtml(String(catalog.year || filters.year))}</th><th>Période</th><th>Contenus</th><th>État</th><th>Préparation QV</th><th>Action</th></tr></thead>
        <tbody>${activities.map((row) => {
          ...
          return `<tr><td>${escapeHtml(row.domain)}</td><td><strong>${escapeHtml(row.label)}</strong><small>${escapeHtml(row.code)}</small></td><td>${required ? `${escapeHtml(String(required))} occurrence${required > 1 ? 's' : ''}` : 'Non défini'}</td><td>${required ? escapeHtml(L.annualPeriodLabel(row.windowStart,row.windowEnd)) : '—'}</td><td>${escapeHtml(contents)}</td><td>${annualStatusHtml(row.status)}</td><td>${escapeHtml(qv)}</td><td><a href="#/quo-vadis/catalogue-annuel/${encodeURIComponent(row.code)}?annee=${encodeURIComponent(catalog.year || filters.year)}">Consulter</a></td></tr>`;
  ...
  function renderAnnualCatalogActivity() {
    ...
    const editable = canManage && requirement && requirement.status === 'DRAFT';
    const groupedThemes = new Map();
    assignments.forEach((row) => {
      const number = Number(row.occurrenceNumber || row.occurrence_number);
      if (!groupedThemes.has(number)) groupedThemes.set(number,[]);
      groupedThemes.get(number).push(row);
    });
    const themeRows = required ? Array.from({ length: required },(_,index) => {
      const occurrence = index + 1;
      const rows = groupedThemes.get(occurrence) || [];
      ...
      return `<tr><td>${occurrence}</td><td>${labels}
        ${editable ? `<div class="annual-theme-editor"><label>Thème existant<select data-annual-theme-select="${occurrence}"><option value="">Choisir…</option>${canonicalOptions}</select></label><span>ou</span><label>Thème libre<input data-annual-theme-free="${occurrence}" maxlength="160" placeholder="Ex. Manœuvre hydraulique"></label><button type="button" class="scope-text-action" data-annual-theme-add="${occurrence}">Ajouter</button></div>` : ''}
      </td></tr>`;
    ...
    return `<div class="annual-activity annual-activity-c8">
      <nav class="annual-breadcrumb" aria-label="Fil d’Ariane">QUO VADIS ${escapeHtml(String(year))} / Catalogue annuel / ${escapeHtml(activity.label || '')}</nav>
      <a class="annual-back" href="#/quo-vadis/catalogue-annuel">‹ Retour au catalogue</a>
      <header><div><span>${escapeHtml(activity.domain || '')}</span><h2>${escapeHtml(activity.label || activity.code || '')}</h2>${activity.version && activity.version.description ? `<p>${escapeHtml(activity.version.description)}</p>` : ''}</div><div class="annual-head-state"><label>Année<input type="number" value="${escapeHtml(String(year))}" disabled></label>${annualStatusHtml(status)}<small>${status === 'DRAFT' ? 'Besoin annuel en cours de définition.' : status === 'READY' ? 'Besoin annuel figé et prêt pour la préparation.' : 'Besoin annuel non défini.'}</small></div></header>
      <div class="annual-summary" aria-label="Synthèse annuelle"><strong>${required || '—'} occurrence${required === 1 ? '' : 's'}</strong><span>${required ? escapeHtml(L.annualPeriodLabel(requirement.windowStart,requirement.windowEnd)) : 'Période à définir'}</span><span>${required ? `${themeSummary.defined} thème${themeSummary.defined > 1 ? 's' : ''} défini${themeSummary.defined > 1 ? 's' : ''}` : 'Contenus à définir'}</span><span>${themeSummary.missing} occurrence${themeSummary.missing > 1 ? 's' : ''} à préciser</span><span>${prepared}/${required || 0} préparée${required > 1 ? 's' : ''} dans QUO VADIS</span></div>
      <div class="annual-workspace">
        <section class="annual-moa"><h3>Besoin annuel ${escapeHtml(String(year))}</h3>
          <form id="annual-requirement-form"><div class="annual-form-grid">
            ...
            <label>Début souhaité<input id="annual-window-start" type="date" placeholder="24/09/2026" value="${escapeHtml(requirement && requirement.windowStart || '')}" ${!canManage || requirement && requirement.status !== 'DRAFT' ? 'disabled' : ''}></label>
            <label>Fin souhaitée<input id="annual-window-end" type="date" placeholder="24/09/2026" value="${escapeHtml(requirement && requirement.windowEnd || '')}" ${!canManage || requirement && requirement.status !== 'DRAFT' ? 'disabled' : ''}></label>
            ...
            <label>Variante<input id="annual-variant-code" value="${escapeHtml(requirement && requirement.variantCode || 'DEFAULT')}" ${!canManage || requirement && requirement.status !== 'DRAFT' ? 'disabled' : ''}></label>
          </div><p class="annual-content-count">Contenus : ${escapeHtml(themeSummary.label)}</p><div class="annual-actions">
            ${canManage && (!requirement || requirement.status === 'DRAFT') ? '<button class="scope-button annual-button-draft" type="submit">Enregistrer le brouillon</button>' : ''}
            ${canManage && requirement && requirement.status === 'DRAFT' ? '<button id="annual-ready" class="scope-button annual-button-confirm" type="button">Valider pour QUO VADIS</button>' : ''}
            ${canManage && requirement && requirement.status === 'READY' ? '<button id="annual-generate" class="scope-button annual-button-confirm" type="button">Générer les occurrences</button><button id="annual-revise" class="scope-button" type="button">Créer une révision</button>' : ''}
          </div></form>
        </section>
        <section class="annual-qv-preparation"><h3>Préparation QUO VADIS</h3><strong>${prepared} / ${required || 0} occurrences préparées</strong><p>Aucune date fixée dans le catalogue.</p><p>Activité, occurrences, thèmes, période, publics et contraintes utiles seront transmis. Dates, horaires, lieux, responsables nommés, arbitrages et publication restent décidés dans QUO VADIS.</p>${requirement && requirement.status === 'READY' ? '<button id="annual-preview" class="scope-button" type="button">Voir la préparation</button>' : ''}</section>
      </div>
      <section class="annual-themes"><h3>Contenus ${escapeHtml(String(year))}</h3><div class="annual-table-wrap"><table class="annual-table"><thead><tr><th>Occurrence</th><th>Thème(s)</th></tr></thead><tbody>${themeRows}</tbody></table></div></section>
      ...
      <div class="annual-reference-grid"><section><h3>Cadre de l’activité</h3><dl><dt>Domaine</dt><dd>${escapeHtml(activity.domain || '—')}</dd>${activity.familyCode ? `<dt>Famille</dt><dd>${escapeHtml(activity.familyCode)}</dd>` : ''}<dt>Public de référence</dt><dd>${escapeHtml(publicLabels.join(', ') || 'À définir')}</dd><dt>Périodicité</dt><dd>${escapeHtml(L.annualEnumLabel(config.periodicity && config.periodicity.periodicity_type || 'ON_DEMAND'))}</dd><dt>Organisation</dt><dd>${escapeHtml(sessionLabels.join(' · ') || 'À définir')}</dd>${activity.version && activity.version.description ? `<dt>Description</dt><dd>${escapeHtml(activity.version.description)}</dd>` : ''}</dl></section>
      ...
      ${canManage ? `<details class="annual-internal"><summary>Détails internes</summary><dl><dt>Code</dt><dd>${escapeHtml(activity.code || '—')}</dd><dt>Version</dt><dd>${escapeHtml(activity.version && activity.version.versionCode || '—')}</dd><dt>Identifiant</dt><dd>${escapeHtml(activity.definitionId || '—')}</dd><dt>Fingerprint</dt><dd>${escapeHtml(activity.version && activity.version.fingerprint || '—')}</dd><dt>Type interne</dt><dd>${escapeHtml(activity.activityType || '—')}</dd></dl></details>` : ''}
    </div>`;
  }
          ...
          : view === 'activite' ? renderQuoVadisActivite(qv)
            : view === 'a-arbitrer' ? renderQuoVadisArbitrer(qv)
              : view === 'alertes' ? renderQuoVadisAlertes(qv)
                : view === 'cursus' ? renderQuoVadisCursus(qv)
  ...
  function render() {
    ...
    const body = r.screen === 'accueil' ? renderAccueil()
      ...
      : r.screen === 'personnel' ? renderPersonnel()
        ...
        : r.screen === 'quo-vadis' ? renderQuoVadis()
  ...
  function bind() {
    ...
    document.getElementById('scope-feedback-cancel')?.addEventListener('click', () => {
      ...
      state.eventSaveBusy = false;
      state.deleteEventBusy = false;
      ScopeFeedback.clear();
    });
    ...
    const annualThemePayload = (rows) => (rows || []).map((row) => ({
      occurrenceNumber: Number(row.occurrenceNumber || row.occurrence_number),
      themeVersionId: row.themeVersionId || row.theme_version_id || null,
      freeLabel: row.freeLabel || row.free_label || null,
      sessionTemplateId: row.sessionTemplateId || row.session_template_id || null,
      sortOrder: Number(row.sortOrder || row.sort_order || 100)
    }));
    document.querySelectorAll('[data-annual-theme-add]').forEach((button) => button.addEventListener('click',async () => {
      const occurrence = Number(button.dataset.annualThemeAdd || 0);
      const select = document.querySelector(`[data-annual-theme-select="${occurrence}"]`);
      const freeInput = document.querySelector(`[data-annual-theme-free="${occurrence}"]`);
      ...
      const requirement = state.annualCatalogActivity && state.annualCatalogActivity.annualRequirement;
      const current = annualThemePayload(state.annualCatalogActivity && state.annualCatalogActivity.annualThemeAssignments);
      const nextSortOrder = Math.max(0,...current.filter((row) => row.occurrenceNumber === occurrence).map((row) => Number(row.sortOrder || 0))) + 1;
      current.push({ occurrenceNumber: occurrence,themeVersionId,freeLabel,sessionTemplateId: null,sortOrder: nextSortOrder });
      await annualAction(() => client.replaceAnnualRequirementThemes(requirement.annualRequirementId || requirement.annual_requirement_id,{ assignments: current }),'Le contenu annuel a été enregistré.');
    }));
    document.querySelectorAll('[data-annual-theme-remove]').forEach((button) => button.addEventListener('click',async () => {
      const assignmentId = button.dataset.annualThemeRemove;
      const requirement = state.annualCatalogActivity && state.annualCatalogActivity.annualRequirement;
      const rows = state.annualCatalogActivity && state.annualCatalogActivity.annualThemeAssignments || [];
      const current = annualThemePayload(rows.filter((row) => String(row.annualThemeAssignmentId || row.annual_theme_assignment_id) !== assignmentId));
      await annualAction(() => client.replaceAnnualRequirementThemes(requirement.annualRequirementId || requirement.annual_requirement_id,{ assignments: current }),'Le thème a été retiré.');
    }));
    ...
    document.querySelectorAll('[data-qv-arbitrer-mode]').forEach((btn) => {
      btn.addEventListener('click', () => {
        state.quoVadisArbitrerMode = btn.getAttribute('data-qv-arbitrer-mode') === 'detaillee' ? 'detaillee' : 'groupes';
        render();
      });
    });
    document.querySelectorAll('[data-qv-arbitrer-group]').forEach((btn) => {
      btn.addEventListener('click', () => {
        const code = btn.getAttribute('data-qv-arbitrer-group') || '';
        if (!code) return;
        const current = Object.assign({}, state.quoVadisArbitrerOpenDomains || {});
        if (!state.quoVadisArbitrerTouched) {
          document.querySelectorAll('[data-qv-arbitrer-group][aria-expanded="true"]').forEach((openBtn) => {
            const openCode = openBtn.getAttribute('data-qv-arbitrer-group');
            if (openCode) current[openCode] = true;
          });
        }
        current[code] = !current[code];
        state.quoVadisArbitrerOpenDomains = current;
        state.quoVadisArbitrerTouched = true;
        render();
      });
    });
    ...
    root.querySelectorAll('[data-qv-open]').forEach((el) => {
      el.addEventListener('click', (event) => {
        ...
        const href = el.getAttribute('data-qv-open');
        if (href) window.location.hash = href;
      });
    });
    root.querySelectorAll('[data-qv-activity-field]').forEach((el) => {
      el.addEventListener('change', async () => {
        const field = el.getAttribute('data-qv-activity-field');
        const id = route().qvActivityId;
        if (!id || !field || typeof client.updateQuoVadisActivity !== 'function') return;
        try {
          const payload = field === 'salleTheorieId'
            ? { salleTheorieId: el.value || null }
            : { responsableFonctionCode: el.value || null };
          const data = await client.updateQuoVadisActivity(id, payload);
          invalidateCache(['quoVadis']);
          state.quoVadis = data.quoVadis || state.quoVadis;
          toast('success', 'Activité', field === 'salleTheorieId' ? 'Salle théorie enregistrée.' : 'Responsable enregistré.');
        } catch (error) {
          toast('error', 'Activité', L.friendlyError(error).message || 'L’enregistrement a échoué.');
        } finally {
          render();
        }
      });
    ...
    document.getElementById('qv-generate')?.addEventListener('click', async () => {
      if (state.quoVadisBusy || typeof client.generateQuoVadisProgramme !== 'function') return;
      state.quoVadisBusy = true;
      render();
      try {
        const data = await client.generateQuoVadisProgramme(2027);
        invalidateCache(['quoVadis']);
        state.quoVadis = data.quoVadis || null;
        state.quoVadisReady = true;
        state.quoVadisGenerationReport = state.quoVadis && state.quoVadis.generation ? state.quoVadis.generation : null;
        const report = state.quoVadisGenerationReport || {};
        toast('success', 'QUO VADIS', `Préparation 2027 mise à jour: ${report.newProposals || 0} nouvelle(s) proposition(s), ${report.attentionPoints || 0} point(s) d’attention.`);
      } catch (error) {
        toast('error', 'QUO VADIS', L.friendlyError(error).message || 'Génération impossible.');
      } finally {
    ...
    root.querySelectorAll('[data-qv-retain]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        ...
        if (!id || typeof client.retainQuoVadisProposal !== 'function') return;
        try {
          const safeId = (typeof CSS !== 'undefined' && CSS.escape) ? CSS.escape(id) : String(id).replace(/"/g, '\\"');
          const lieuSelect = root.querySelector(`[data-qv-proposal-lieu="${safeId}"]`);
          const lieuValue = lieuSelect ? lieuSelect.value : '';
          const data = await client.retainQuoVadisProposal(id, { justification: 'Retenu depuis la préparation annuelle.', lieuId: lieuValue === 'autre' ? '' : lieuValue });
          invalidateCache(['quoVadis']);
          state.quoVadis = data.quoVadis || state.quoVadis;
          toast('success', 'Proposition retenue', 'La date est retenue pour la préparation, sans création d’événement opérationnel.');
        } catch (error) {
          toast('error', 'Arbitrage', L.friendlyError(error).message || 'La proposition n’a pas pu être retenue.');
        } finally {
          render();
        }
      });
    ...
    root.querySelectorAll('[data-qv-cursus-plan]').forEach((input) => {
      input.addEventListener('change', async () => {
        ...
        const year = Number(((state.quoVadis || {}).programme || {}).annee || 2027);
        try {
          const data = await client.setQuoVadisCursus(year, { code, retenu, justification: retenu ? `Retenu pour le programme ${year}.` : `Non retenu pour le programme ${year}.` });
          invalidateCache(['quoVadis']);
          state.quoVadis = data.quoVadis || state.quoVadis;
          toast('success', 'Cursus', retenu ? 'Cursus retenu.' : `Cursus écarté pour ${year}.`);
        } catch (error) {
    ...
    document.getElementById('qv-cursus-new')?.addEventListener('click', () => {
      state.quoVadisNewCursus = true;
      render();
      document.getElementById('qv-cursus-name')?.focus();
    });
    document.getElementById('qv-cursus-cancel')?.addEventListener('click', () => {
      state.quoVadisNewCursus = false;
      render();
    });
    document.getElementById('qv-cursus-domain')?.addEventListener('change', (event) => {
      const rows = L.eventCiblesForForm(event.target.value, ((state.quoVadis || {}).cibles || []), []);
      document.getElementById('qv-cursus-target').innerHTML = sharedOptionsHtml([{ value: '', label: 'Aucun public particulier' }].concat(rows.map((row) => ({ value: row.cibleId, label: row.libelle || row.niveauCode }))), '');
    });
    document.getElementById('qv-cursus-create')?.addEventListener('submit', async (event) => {
      event.preventDefault();
      const libelle = String(document.getElementById('qv-cursus-name')?.value || '').trim();
      if (!libelle) return;
      try {
        const data = await client.createQuoVadisCursus({ libelle, code: document.getElementById('qv-cursus-code')?.value, domainCode: document.getElementById('qv-cursus-domain')?.value, cibleId: document.getElementById('qv-cursus-target')?.value || null, durationMonths: document.getElementById('qv-cursus-duration')?.value, description: document.getElementById('qv-cursus-description')?.value });
        invalidateCache(['quoVadis']);
        await loadQuoVadis();
        state.quoVadisCursusCode = data.cursus.code;
        state.quoVadisCursusQuery = '';
        state.quoVadisNewCursus = false;
        state.quoVadisCursusFeedback = 'Cursus créé.';
        window.setTimeout(() => { if (state.quoVadisCursusFeedback === 'Cursus créé.') { state.quoVadisCursusFeedback = ''; render(); } }, 3500);
      } catch (error) {
        toast('error', 'Cursus', L.friendlyError(error).message || 'Le cursus n’a pas pu être créé.');
      } finally {
        render();
      }
    });
    const qvRefreshReference = async (message) => {
      invalidateCache(['quoVadis']);
      await loadQuoVadis();
      state.quoVadisCursusFeedback = message;
      render();
      window.setTimeout(() => { if (state.quoVadisCursusFeedback === message) { state.quoVadisCursusFeedback = ''; render(); } }, 3500);
    };
    document.getElementById('qv-cursus-edit')?.addEventListener('click', () => { state.quoVadisCursusEdit = state.quoVadisCursusCode; render(); });
    document.getElementById('qv-cursus-edit-cancel')?.addEventListener('click', () => { state.quoVadisCursusEdit = ''; render(); });
    document.getElementById('qv-cursus-edit-domain')?.addEventListener('change', (event) => {
      const domain = event.target.value;
      const rows = L.eventCiblesForForm(domain, ((state.quoVadis || {}).cibles || []), []);
      document.getElementById('qv-cursus-edit-target').innerHTML = sharedOptionsHtml([{ value: '', label: 'Aucun public particulier' }].concat(rows.map((row) => ({ value: row.cibleId, label: row.libelle || row.niveauCode }))), '');
    });
    document.getElementById('qv-cursus-edit-form')?.addEventListener('submit', async (event) => {
      event.preventDefault();
      const selected = ((state.quoVadis || {}).cursusSelections || []).find((row) => row.code === state.quoVadisCursusCode);
      if (!selected) return;
      try {
        await client.updateQuoVadisCursus(selected.cursusId, { libelle: document.getElementById('qv-cursus-edit-name').value, durationMonths: document.getElementById('qv-cursus-edit-duration').value, domainCode: document.getElementById('qv-cursus-edit-domain').value, cibleId: document.getElementById('qv-cursus-edit-target').value || null, description: document.getElementById('qv-cursus-edit-description').value, statut: document.getElementById('qv-cursus-edit-status').value });
        state.quoVadisCursusEdit = '';
        await qvRefreshReference('Cursus mis à jour.');
      } catch (error) { state.quoVadisCursusFeedback = L.friendlyError(error).message; render(); }
    });
    document.getElementById('qv-cursus-delete')?.addEventListener('click', async () => {
      const selected = ((state.quoVadis || {}).cursusSelections || []).find((row) => row.code === state.quoVadisCursusCode);
      if (!selected || !window.confirm(`Archiver ou supprimer ${selected.libelle} selon ses liens historiques ?`)) return;
      try { const result = await client.deleteQuoVadisCursus(selected.cursusId); await qvRefreshReference(result.archived ? 'Cursus archivé ; historique conservé.' : 'Cursus supprimé.'); }
      catch (error) { state.quoVadisCursusFeedback = L.friendlyError(error).message; render(); }
    });
    document.getElementById('qv-module-new')?.addEventListener('click', () => { state.quoVadisModuleNew = true; state.quoVadisModuleEdit = ''; render(); });
    document.getElementById('qv-module-cancel')?.addEventListener('click', () => { state.quoVadisModuleNew = false; state.quoVadisModuleEdit = ''; render(); });
    root.querySelectorAll('[data-qv-module-edit]').forEach((button) => button.addEventListener('click', () => { state.quoVadisModuleEdit = button.dataset.qvModuleEdit; state.quoVadisModuleNew = false; render(); }));
    document.getElementById('qv-module-form')?.addEventListener('submit', async (event) => {
      event.preventDefault();
      const selected = ((state.quoVadis || {}).cursusSelections || []).find((row) => row.code === state.quoVadisCursusCode);
      const stepId = event.target.dataset.stepId;
      const body = { libelle: document.getElementById('qv-module-name').value, logicalYear: document.getElementById('qv-module-year').value, durationMinutes: document.getElementById('qv-module-minutes').value, preferredMonth: document.getElementById('qv-module-month').value || null, usualStartTime: document.getElementById('qv-module-start').value || null, usualEndTime: document.getElementById('qv-module-end').value || null, lieuId: document.getElementById('qv-module-place').value || null, cibleId: document.getElementById('qv-module-target').value || null };
      try {
        if (stepId) await client.updateQuoVadisModule(stepId, body);
        else await client.createQuoVadisModule(selected.cursusId, body);
        state.quoVadisModuleEdit = ''; state.quoVadisModuleNew = false;
        await qvRefreshReference('Module enregistré.');
      } catch (error) { state.quoVadisCursusFeedback = L.friendlyError(error).message; render(); }
    });
    root.querySelectorAll('[data-qv-module-delete]').forEach((button) => button.addEventListener('click', async () => {
      if (!window.confirm('Archiver ou supprimer ce module selon ses liens historiques ?')) return;
      try { const result = await client.deleteQuoVadisModule(button.dataset.qvModuleDelete); await qvRefreshReference(result.archived ? 'Module archivé ; historique conservé.' : 'Module supprimé.'); }
      catch (error) { state.quoVadisCursusFeedback = L.friendlyError(error).message; render(); }
    }));
    root.querySelectorAll('[data-qv-module-move]').forEach((button) => button.addEventListener('click', async () => {
      const selected = ((state.quoVadis || {}).cursusSelections || []).find((row) => row.code === state.quoVadisCursusCode);
      const ids = ((state.quoVadis || {}).cursus || []).filter((row) => row.code === selected.code && row.stepId && row.active).sort((a, b) => a.ordre - b.ordre).map((row) => row.stepId);
      const from = ids.indexOf(button.dataset.qvModuleMove), to = from + Number(button.dataset.direction);
      if (from < 0 || to < 0 || to >= ids.length) return;
      [ids[from], ids[to]] = [ids[to], ids[from]];
      try { await client.reorderQuoVadisModules(selected.cursusId, ids); await qvRefreshReference('Ordre des modules enregistré.'); }
      catch (error) { state.quoVadisCursusFeedback = L.friendlyError(error).message; render(); }
    }));
    root.querySelectorAll('[data-qv-step-schedule]').forEach((button) => button.addEventListener('click', () => { state.quoVadisScheduleStepId = button.dataset.qvStepSchedule; render(); }));
    document.getElementById('qv-step-schedule-cancel')?.addEventListener('click', () => { state.quoVadisScheduleStepId = ''; render(); });
    document.getElementById('qv-step-schedule-form')?.addEventListener('submit', async (event) => {
      event.preventDefault();
      const year = Number(((state.quoVadis || {}).programme || {}).annee || 2027);
      const body = { stepId: event.target.dataset.stepId, date: document.getElementById('qv-step-date').value, start: document.getElementById('qv-step-start').value, end: document.getElementById('qv-step-end').value, durationMinutes: document.getElementById('qv-step-duration').value, lieuId: document.getElementById('qv-step-place').value || null };
      try { await client.setQuoVadisCursusStepSchedule(year, body); state.quoVadisScheduleStepId = ''; await qvRefreshReference('Occurrence annuelle enregistrée.'); }
      catch (error) { state.quoVadisCursusFeedback = L.friendlyError(error).message; render(); }
    });
    ...
    document.getElementById('qv-rules-edit')?.addEventListener('click', () => {
      state.quoVadisRuleEditor = 'new'; render();
    });
    document.getElementById('qv-rule-cancel')?.addEventListener('click', () => { state.quoVadisRuleEditor = ''; render(); });
    root.querySelectorAll('[data-qv-rule-edit]').forEach((button) => button.addEventListener('click', () => { state.quoVadisRuleEditor = button.dataset.qvRuleEdit; render(); }));
    document.getElementById('qv-rule-domain')?.addEventListener('change', (event) => {
      const rows = L.eventCiblesForForm(event.target.value, ((state.quoVadis || {}).cibles || []), []);
      document.getElementById('qv-rule-target').innerHTML = sharedOptionsHtml([{ value: '', label: 'Règle générale' }].concat(rows.map((row) => ({ value: row.cibleId, label: row.libelle || row.niveauCode }))), '');
    });
    document.getElementById('qv-rule-form')?.addEventListener('submit', async (event) => {
      event.preventDefault();
      ...
      try {
        ...
        invalidateCache(['quoVadis']); await loadQuoVadis();
        state.quoVadisRuleEditor = ''; state.quoVadisRulesFeedback = 'Règle enregistrée.'; render();
        window.setTimeout(() => { if (state.quoVadisRulesFeedback === 'Règle enregistrée.') { state.quoVadisRulesFeedback = ''; render(); } }, 3500);
      } catch (error) { state.quoVadisRulesFeedback = L.friendlyError(error).message; render(); }
    });
    root.querySelectorAll('[data-qv-rule-delete]').forEach((button) => button.addEventListener('click', async () => {
      if (!window.confirm('Archiver cette règle ?')) return;
      try { await client.deleteQuoVadisRule(button.dataset.qvRuleDelete); invalidateCache(['quoVadis']); await loadQuoVadis(); state.quoVadisRulesFeedback = 'Règle archivée.'; render(); window.setTimeout(() => { if (state.quoVadisRulesFeedback === 'Règle archivée.') { state.quoVadisRulesFeedback = ''; render(); } }, 3500); }
      catch (error) { state.quoVadisRulesFeedback = L.friendlyError(error).message; render(); }
    }));
    ...
    root.querySelectorAll('[data-qv-cursus-step]').forEach((input) => {
      input.addEventListener('change', async () => {
        ...
        const year = Number(((state.quoVadis || {}).programme || {}).annee || 2027);
        try {
          const data = await client.setQuoVadisCursusStep(year, { stepId, retenu: Boolean(input.checked) });
          invalidateCache(['quoVadis']);
          state.quoVadis = data.quoVadis || state.quoVadis;
    ...
    document.getElementById('qv-export-pdf')?.addEventListener('click', async () => {
      ...
      const rows = qvExportRows(qvFilteredActivities(quoVadisData()));
      const meta = qvActiveFiltersMeta();
      try {
        const result = await client.generateQuoVadisReport({ rows, meta });
    ...
    const syncFutureForm = (rerender) => {
      const previousLieu = (state.quoVadisFutureForm || {}).lieuId;
      state.quoVadisFutureForm = {
        dateDebut: document.getElementById('qv-future-date')?.value || '',
        heureDebut: document.getElementById('qv-future-start')?.value || '',
        dateFin: document.getElementById('qv-future-end-date')?.value || '',
        heureFin: document.getElementById('qv-future-end')?.value || '',
    ...
    document.getElementById('qv-future-domain')?.addEventListener('change', () => {
      ...
      const oiOk = (L.eventCiblesForForm ? L.eventCiblesForForm(form.domain, ((state.quoVadis || {}).cibles || []), []) : []).some((row) => String(row.niveauCode) === String(canonicalOi));
      const specOk = (L.sharedSpecOptions ? L.sharedSpecOptions(form.domain) : []).some((row) => String(row.value) === String(form.specialisation || ''));
      const cursusOk = ((L.domainTaxonomyGroups().find((group) => group.label === 'Formation') || {}).codes || []).includes(form.domain)
        && ((state.quoVadis || {}).cursusSelections || []).some((row) => row.retenu && String(row.cursusId) === String(form.cursusId));
    ...
    document.getElementById('qv-future-save')?.addEventListener('click', async () => {
      ...
      if (!form.heureDebut) errors.heureDebut = 'L’heure de début est obligatoire.';
      if (!String(form.activiteLabel || '').trim()) errors.activiteLabel = 'L’activité est obligatoire.';
      const endDate = form.dateFin || form.dateDebut;
      if (form.dateFin && form.dateDebut && form.dateFin < form.dateDebut) {
        errors.dateFin = 'La fin doit être postérieure ou égale au début.';
      }
      if (form.heureFin && form.heureDebut && endDate && `${endDate}T${form.heureFin}` < `${form.dateDebut}T${form.heureDebut}`) {
        errors.heureFin = 'La fin doit être postérieure ou égale au début.';
      }
      state.quoVadisFutureErrors = errors;
      if (Object.keys(errors).length) {
        render();
        return;
      }
      try {
        const payload = Object.assign({ targetYear: Number(((state.quoVadis || {}).programme || {}).annee || 2027), autreLieu: form.lieuId === 'autre' }, form);
    ...
    document.getElementById('edit-event-ok')?.addEventListener('click', () => {
      if (state.eventSaveBusy) return;
      const id = route().id;
      const form = state.editEventForm || {};
      const cibleIds = form.cibleIds || [];
      if (!cibleIds.length) {
        ScopeFeedback.error('Cible obligatoire', 'Choisissez au moins une cible.');
        return;
      }
      if (form.statut === 'REPORTE' && !String(form.motif || '').trim()) {
        ScopeFeedback.error('Motif obligatoire', 'Indiquez le motif du report.');
        return;
      }
      const btn = document.getElementById('edit-event-ok');
      state.eventSaveBusy = true;
      if (btn) {
        btn.disabled = true;
      ...
      (async () => {
        try {
          ...
          state.eventSaveBusy = false;
          await loadFiche(id);
          ScopeFeedback.success('Événement mis à jour', 'Les modifications ont été enregistrées.');
        } catch (error) {
          state.eventSaveBusy = false;
          const info = friendlyActionError(error);
          ...
          if (btn) {
            btn.disabled = false;
            btn.textContent = form.confirmed ? 'Confirmer et enregistrer' : 'Enregistrer les modifications';
          }
        }
      })();
    });
    ...
    document.getElementById('preview-q')?.addEventListener('input', (e) => {
      ...
      searchPersonnes(e.target.value, 'preview');
    });
    root.querySelectorAll('[data-add-ex]').forEach((btn) => {
      btn.addEventListener('click', () => addPreviewPerson(btn.getAttribute('data-add-ex')));
    });
    ...
    root.querySelectorAll('[data-time-mode-toggle]').forEach((btn) => {
      btn.addEventListener('click', () => {
        const tr = btn.closest('[data-pid]');
        if (!tr) return;
        const row = state.saisie.find((r) => r.personneId === tr.getAttribute('data-pid'));
        if (!row || (L.sessionLocked && L.sessionLocked(row))) return;
        if (btn.getAttribute('data-time-mode-toggle') === 'CUSTOM') {
          row.timeOverrideOpen = true;
          row.timeFieldsOpen = true;
        } else {
          row.timeOverrideOpen = false;
          row.timeFieldsOpen = false;
    ...
    document.getElementById('cancel-ok')?.addEventListener('click', () => {
      ...
      withFeedbackAction({
        ...
        successMessage: 'Événement annulé'
      }, async () => {
    ...
    document.getElementById('delete-ok')?.addEventListener('click', () => {
      if (state.deleteEventBusy) return;
      const id = route().id;
      state.deleteEventBusy = true;
      const btn = document.getElementById('delete-ok');
      if (btn) btn.disabled = true;
      withFeedbackAction({
        progressTitle: 'Suppression…',
        successTitle: 'Événement supprimé',
        successMessage: 'Événement supprimé'
      }, async () => {
        try {
          await client.masquer(id, 'Suppression métier des vues opérationnelles', state.fiche.evenement.version);
          invalidateCache(['list', 'dashboard', 'vigilance', 'cycles']);
          state.modal = null;
          await loadList();
          go('#/exercices');
  ...
  function bindPersonnelSync() {
    ...
    root.querySelectorAll('[data-scope-sort][data-sort-key]').forEach((th) => {
      th.addEventListener('click', () => {
        ...
        if (table === 'qv-arbitrer') {
          state.quoVadisArbitrerSort = L.nextSort ? L.nextSort(state.quoVadisArbitrerSort, key, key === 'date' || key === 'horaire' ? 'asc' : 'asc') : { key, dir: 'asc' };
          render();
        }
      });
    });
    ...
    document.getElementById('participation-policy-save')?.addEventListener('click', () => {
      ...
      withLoading(async () => {
        ...
        ScopeFeedback.success('Politique enregistrée', 'La politique de participation a été enregistrée.');
      });
    });
    ...
    const resetFormationDefinitionForm = (options = {}) => {
      const nextYear = String(new Date().getFullYear() + 1);
      const creationDefaults = options.creation === true;
      state.formationDefinitionForm = {
        domain: creationDefaults ? '' : '',
        label: '',
        description: '',
        modeOrganisation: 'SIMPLE',
        sessionCount: '1',
    ...
    const focusFormationForm = () => {
      setTimeout(() => {
        const el = document.getElementById('formation-editor');
        if (!el) return;
    ...
    document.getElementById('formation-open-create')?.addEventListener('click', () => {
    ...
    document.getElementById('formation-create')?.addEventListener('click', () => {
      const form = state.formationDefinitionForm || {};
      withLoading(async () => {
        const checked = (kind) => Array.from(root.querySelectorAll(`[data-formation-policy^="${kind}:"]`))
          .filter((input) => input.checked)
          .map((input) => String(input.getAttribute('data-formation-policy') || '').split(':')[1])
          .filter(Boolean);
        ...
        const savedConfiguration = await client.createEventDefinition({
          ...
          policyConfig: {
            activeStatuses: ['NON_RENSEIGNE', ...selectedStatusesPayload],
            excuseMotifs: checked('excuse'),
            dispenseMotifs: checked('dispense')
          },
    ...
    document.getElementById('formation-cancel-edit')?.addEventListener('click', () => {
      ...
      ScopeFeedback.confirm({
        ...
        cancelText: 'Continuer la modification',
        tone: 'warning'
      }, abandon);
    });
    root.querySelectorAll('[data-referential-add]').forEach((btn) => {
      btn.addEventListener('click', () => {
        state.formationReferentialDraft = {
          type: btn.getAttribute('data-referential-add'),
          label: '',
          baseStatus: 'PRESENT',
          order: '999',
          active: true
    ...
    ['referential-label', 'referential-base-status', 'referential-order', 'referential-active'].forEach((id) => {
      document.getElementById(id)?.addEventListener(id === 'referential-active' ? 'change' : 'input', (e) => {
        if (!state.formationReferentialDraft) return;
        const key = id === 'referential-label' ? 'label' : (id === 'referential-base-status' ? 'baseStatus' : (id === 'referential-order' ? 'order' : 'active'));
        state.formationReferentialDraft[key] = id === 'referential-active' ? e.target.checked : e.target.value;
      });
    });
    document.getElementById('referential-cancel')?.addEventListener('click', () => {
      state.formationReferentialDraft = null;
      render();
    });
      ...
      withFeedbackAction({
        ...
        if (draft.type === 'status') {
          await client.saveParticipationStatus({
            ...
            displayOrder: Number(draft.order || 999)
          });
        } else {
        ...
        ScopeFeedback.confirm({
          ...
          if (type === 'status') {
            await client.saveParticipationStatus({
              ...
              active,
              historique: true,
              displayOrder: Number(row.order || row.display_order || 999)
            });
          } else {
            await client.saveParticipationMotif({
              motifId: id,
              motifType: type === 'dispense' ? 'DISPENSE' : 'EXCUSE',
              label: row.label || row.libelle || id,
              active,
              historique: true,
              displayOrder: Number(row.order || row.display_order || 999),
    ...
    root.querySelectorAll('[data-reconduct-definition-version]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        withLoading(async () => {
          ...
          invalidateCache(['referentiels', 'formationCatalog']);
          await loadFormationCatalog();
          ScopeFeedback.success('Reconduction créée', `Une version ${year} a été préparée.`);
        });
      });
    });
  ...
  function generateEventReport(evenementId) {
    const fiche = state.fiche && String(state.fiche.evenement && state.fiche.evenement.evenement_id) === String(evenementId)
      ? state.fiche
      : ((state.list || []).find((item) => String(item.evenement && item.evenement.evenement_id) === String(evenementId)) || null);
    const ev = (fiche && fiche.evenement) || { evenement_id: evenementId };
    const availability = eventReportAvailability(ev, fiche);
    if (!availability.ok) {
      ScopeFeedback.info('Rapport indisponible', availability.reason || 'Le rapport n’est pas encore exploitable.');
      return;
    }
  ...
  function statComPdfFilterMeta() {
    ...
    return [
      ['Recherche', state.formationStatComQuery ? state.formationStatComQuery : 'Toutes'],
      ['Domaine', state.formationStatComDomainFilter || 'Tous'],
      ['État', stateFilter === 'TOUS' ? 'Tous' : stateFilter === 'ACTIF' ? 'Actifs' : 'Inactifs'],
      ['Tri', L.statComSortLabel ? L.statComSortLabel(state.formationStatComSort) : 'Code — croissant']
    ];
  }
  ...
  function searchPersonnes(value, kind) {
    ...
    const targetId = kind === 'encadrement' ? 'enc-suggestions' : (kind === 'preview' ? 'preview-suggestions' : 'manual-person-suggestions');
    ...
    state.scopeSearchTimers[kind] = setTimeout(() => {
      ...
      const hits = (data.personnes || []).filter((p) => {
        ...
        if (kind === 'encadrement') {
          ...
          state.personHits = sortPeopleForEncadrement(hits);
          renderSuggestionList(kind, state.personHits);
        } else {
        ...
        if (state.scopeSearchTokens[kind] !== token) return;
        const info = L.friendlyError(error);
        ScopeFeedback.error(info.title, info.message);
      });
  ...
  function renderSuggestionList(kind, rows) {
    ...
    target.querySelectorAll('[data-add-ex]').forEach((btn) => {
      btn.addEventListener('click', () => addPreviewPerson(btn.getAttribute('data-add-ex')));
    });
  ...
  async function onRoute() {
    ...
    if (r.screen === 'quo-vadis' && r.qvView === 'activite' && r.qvActivityId && state.quoVadisHistoryId !== String(r.qvActivityId)) {
      state.quoVadisHistoryReady = false;
      state.quoVadisHistory = [];
      state.quoVadisHistoryError = null;
    }
    ...
    await withLoading(async () => {
      ...
      if (r.screen === 'liste' || r.screen === 'rapports' || r.screen === 'accueil') jobs.push(loadList());
      if (r.screen === 'rapport-jsp' || r.screen === 'rapport-participation') jobs.push(loadJspReport());
      if (r.screen === 'rapport-formation') jobs.push(loadFormationReport());
      if (r.screen === 'cycles') jobs.push(loadCycles());
      if (r.screen === 'cycle' && r.id) jobs.push(loadCycle(r.id));
      if (r.screen === 'vigilance') jobs.push(loadVigilance());
      if (r.screen === 'vue' || r.screen === 'accueil' || r.screen === 'statistiques') jobs.push(loadDashboard());
      if (r.screen === 'personnel' || r.screen === 'import-personnel') {
        jobs.push(loadPersonCount());
        jobs.push(loadPersonnelDirectory());
      }
  ...
  if (window.__SCOPE_UI_TEST_HOOKS__) {
    window.ScopeUiTestHooks = {
      ...
      renderFormationCatalogHtml(payload, selectedVersionId) {
        state.referentiels = Object.assign({}, state.referentiels || {}, {
        ...
        state.formationCatalogReady = true;
        state.formationCatalogError = null;
        state.formationSelectedVersionId = selectedVersionId || '';

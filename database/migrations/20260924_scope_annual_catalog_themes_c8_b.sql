-- SCOPE C8-B annual catalogue themes. Additive only, no seed or backfill.
begin;

select pg_advisory_xact_lock(671902283);
...
create or replace function scope_normalize_theme_label(value text)
...
insert into monitoring_f7_schema_migrations(version) values ('scope-annual-catalog-themes-c8-b') on conflict (version) do nothing;

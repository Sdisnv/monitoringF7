...
(function () {
  ...
  function roleLabel() {
    ...
    return state.session ? 'Session SCOPE' : 'Session requise';
  }
  ...
  function bannerHtml() {
    ...
    if (liveGate) {
      bits.push(`<div class="scope-banner warning" role="alertdialog">
        ...
        <div>Le paramètre URL ne suffit pas. Confirmez pour utiliser le runtime SCOPE. Les données de démonstration ne doivent pas être envoyées en production.</div>
      ...
      bits.push(`<div class="scope-banner warning" role="alertdialog">
        ...
        <div class="scope-actions">
          <a class="scope-btn scope-btn-primary" id="scope-okta-login" href="${escapeHtml(L.oktaLoginHref('/?mode=live'))}">Se connecter avec Okta</a>
        </div>
      </div>`);
    } else if (mode === 'live') {
      bits.push(`<div class="scope-banner live">Mode LIVE — PostgreSQL SCOPE, session Okta. Toute saisie est réelle.</div>`);
    } else {
      bits.push(`<div class="scope-banner demo">
        <strong>Mode démonstration</strong>
        <div>Mode démonstration — aucune écriture dans PostgreSQL SCOPE. Le personnel affiché est local et fictif.</div>
        <div class="scope-actions">

...
assert.ok(/Prévu en \$\{year\}/.test(cursus) && /Non prévu en \$\{year\}/.test(cursus));
assert.ok(/data-qv-cursus-plan/.test(cursus));
assert.ok(/1\. Planning \$\{year\}/.test(cursus));
assert.ok(/2\. Modules du cursus/.test(cursus));
assert.ok(/Inclus \$\{year\}/.test(cursus));
assert.ok(/data-qv-cursus-step/.test(cursus));
assert.ok(/startYearLabel/.test(cursus));
assert.ok(/Année 1/.test(cursus));
assert.ok(/modules sont décochés/.test(cursus));
...
assert.ok(/createCursus/.test(service) && /setCursusStepSelection/.test(service));
...
assert.ok(/scope-referentiel-cursus-taxonomie-2/.test(schema));
...
assert.ok(/scope-referentiel-cursus-taxonomie-2/.test(html));

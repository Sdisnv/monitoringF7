...
(async () => {
  ...
  await record('07 administration Participation, schema et cache-bust sont presents', () => {
    ...
    assert.ok(/scope\.css\?v=scope-[^"']+/.test(htmlSource));
    assert.ok(/scope-ui-logic\.js\?v=scope-[^"']+/.test(htmlSource));
    assert.ok(/scope-api\.js\?v=scope-[^"']+/.test(htmlSource));
    assert.ok(/scope-ui\.js\?v=scope-[^"']+/.test(htmlSource));
  });

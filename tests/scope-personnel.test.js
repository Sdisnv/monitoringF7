...
12345;Cpl;Marc;DUPONT;XYZ B9`), 'OI')[0];
  ...
  assert.ok(ui.includes('Analyse du fichier en cours'));
  assert.ok(ui.includes('Analyse terminée'));
  ...
  assert.ok(ui.includes("personnelImportCount(counts, 'countErrors', 'ERROR') === 0"));
  ...
  assert.ok(!ui.includes('assets/data/PersonnelSDIS.csv'));
  assert.ok(!ui.includes('PersonnelSDIS.csv'));
